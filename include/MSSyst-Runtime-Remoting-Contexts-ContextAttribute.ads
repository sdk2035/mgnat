-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
with MSSyst.Attribute;
with MSSyst.Runtime.InteropServices.uAttribute;
limited with MSSyst.Runtime.Remoting.Activation.IConstructionCallMessage;
limited with MSSyst.Runtime.Remoting.Contexts.Context;
with MSSyst.Runtime.Remoting.Contexts.IContextAttribute;
with MSSyst.Runtime.Remoting.Contexts.IContextProperty;
limited with MSSyst.String;
limited with MSSyst.Type_k;
package MSSyst.Runtime.Remoting.Contexts.ContextAttribute is
   type Typ is new MSSyst.Attribute.Typ
         and MSSyst.Runtime.InteropServices.uAttribute.Typ
         and MSSyst.Runtime.Remoting.Contexts.IContextAttribute.Typ
         and MSSyst.Runtime.Remoting.Contexts.IContextProperty.Typ
   with record
      null;
   end record;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   function new_ContextAttribute(
      This : Ref := null;
      name : access MSSyst.String.Typ'Class) return Ref;
   function Equals(
      This : access Typ;
      o : access MSSyst.Object.Typ'Class) return Standard.Boolean;
   procedure Freeze(
      This : access Typ;
      newContext : access MSSyst.Runtime.Remoting.Contexts.Context.Typ'Class);
   function get_Name(
      This : access Typ) return access MSSyst.String.Typ'Class;
   function GetHashCode(
      This : access Typ) return Integer;
   procedure GetPropertiesForNewContext(
      This : access Typ;
      ctorMsg : access MSSyst.Runtime.Remoting.Activation.IConstructionCallMessage.Typ'Class);
   function IsContextOK(
      This : access Typ;
      ctx : access MSSyst.Runtime.Remoting.Contexts.Context.Typ'Class;
      ctorMsg : access MSSyst.Runtime.Remoting.Activation.IConstructionCallMessage.Typ'Class) return Standard.Boolean;
   function IsNewContextOK(
      This : access Typ;
      newCtx : access MSSyst.Runtime.Remoting.Contexts.Context.Typ'Class) return Standard.Boolean;
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_ContextAttribute);
   pragma Import(MSIL,Equals,"Equals");
   pragma Import(MSIL,Freeze,"Freeze");
   pragma Import(MSIL,get_Name,"get_Name");
   pragma Import(MSIL,GetHashCode,"GetHashCode");
   pragma Import(MSIL,GetPropertiesForNewContext,"GetPropertiesForNewContext");
   pragma Import(MSIL,IsContextOK,"IsContextOK");
   pragma Import(MSIL,IsNewContextOK,"IsNewContextOK");
end MSSyst.Runtime.Remoting.Contexts.ContextAttribute;
pragma Import(MSIL,MSSyst.Runtime.Remoting.Contexts.ContextAttribute,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[mscorlib]System.Runtime.Remoting.Contexts.ContextAttribute");
