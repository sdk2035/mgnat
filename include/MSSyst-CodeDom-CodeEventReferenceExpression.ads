-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
with MSSyst.CodeDom.CodeExpression;
with MSSyst.CodeDom.CodeObject;
limited with MSSyst.Collections.IDictionary;
limited with MSSyst.String;
limited with MSSyst.Type_k;
package MSSyst.CodeDom.CodeEventReferenceExpression is
   type Typ is new MSSyst.CodeDom.CodeExpression.Typ   with record
      null;
   end record;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   function new_CodeEventReferenceExpression(
      This : Ref := null) return Ref;
   function new_CodeEventReferenceExpression(
      This : Ref := null;
      targetObject : access MSSyst.CodeDom.CodeExpression.Typ'Class;
      eventName : access MSSyst.String.Typ'Class) return Ref;
   function get_EventName(
      This : access Typ) return access MSSyst.String.Typ'Class;
   function get_TargetObject(
      This : access Typ) return access MSSyst.CodeDom.CodeExpression.Typ'Class;
   procedure set_EventName(
      This : access Typ;
      value : access MSSyst.String.Typ'Class);
   procedure set_TargetObject(
      This : access Typ;
      value : access MSSyst.CodeDom.CodeExpression.Typ'Class);
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_CodeEventReferenceExpression);
   pragma Import(MSIL,get_EventName,"get_EventName");
   pragma Import(MSIL,get_TargetObject,"get_TargetObject");
   pragma Import(MSIL,set_EventName,"set_EventName");
   pragma Import(MSIL,set_TargetObject,"set_TargetObject");
end MSSyst.CodeDom.CodeEventReferenceExpression;
pragma Import(MSIL,MSSyst.CodeDom.CodeEventReferenceExpression,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System]System.CodeDom.CodeEventReferenceExpression");
