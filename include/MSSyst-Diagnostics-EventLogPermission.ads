-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
limited with MSSyst.Diagnostics.EventLogPermissionAccess;
limited with MSSyst.Diagnostics.EventLogPermissionEntryCollection;
limited with MSSyst.Diagnostics.EventLogPermissionEntry;
with MSSyst.Security.CodeAccessPermission;
with MSSyst.Security.IPermission;
with MSSyst.Security.ISecurityEncodable;
with MSSyst.Security.IStackWalk;
with MSSyst.Security.Permissions.IUnrestrictedPermission;
limited with MSSyst.Security.Permissions.PermissionState;
with MSSyst.Security.Permissions.ResourcePermissionBase;
limited with MSSyst.Security.SecurityElement;
limited with MSSyst.String;
limited with MSSyst.Type_k;
package MSSyst.Diagnostics.EventLogPermission is
   type Typ is new MSSyst.Security.Permissions.ResourcePermissionBase.Typ
         and MSSyst.Security.IPermission.Typ
         and MSSyst.Security.ISecurityEncodable.Typ
         and MSSyst.Security.IStackWalk.Typ
         and MSSyst.Security.Permissions.IUnrestrictedPermission.Typ
   with record
      null;
   end record;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   function new_EventLogPermission(
      This : Ref := null) return Ref;
   function new_EventLogPermission(
      This : Ref := null;
      state : MSSyst.Security.Permissions.PermissionState.Valuetype) return Ref;
   function new_EventLogPermission(
      This : Ref := null;
      permissionAccess : MSSyst.Diagnostics.EventLogPermissionAccess.Valuetype;
      machineName : access MSSyst.String.Typ'Class) return Ref;
   function new_EventLogPermission(
      This : Ref := null;
      permissionAccessEntries : access MSSyst.Diagnostics.EventLogPermissionEntry.Ref_arr) return Ref;
   function get_PermissionEntries(
      This : access Typ) return access MSSyst.Diagnostics.EventLogPermissionEntryCollection.Typ'Class;
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_EventLogPermission);
   pragma Import(MSIL,get_PermissionEntries,"get_PermissionEntries");
end MSSyst.Diagnostics.EventLogPermission;
pragma Import(MSIL,MSSyst.Diagnostics.EventLogPermission,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System]System.Diagnostics.EventLogPermission");
