-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
limited with MSSyst.Net.ICredentials;
limited with MSSyst.Uri;
package MSSyst.Net.IWebProxy is
   type Typ is interface;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   function get_Credentials(
      This : access Typ) return access MSSyst.Net.ICredentials.Typ'Class is abstract;
   function GetProxy(
      This : access Typ;
      destination : access MSSyst.Uri.Typ'Class) return access MSSyst.Uri.Typ'Class is abstract;
   function IsBypassed(
      This : access Typ;
      host : access MSSyst.Uri.Typ'Class) return Standard.Boolean is abstract;
   procedure set_Credentials(
      This : access Typ;
      value : access MSSyst.Net.ICredentials.Typ'Class) is abstract;
private
   pragma Import(MSIL,get_Credentials,"get_Credentials");
   pragma Import(MSIL,GetProxy,"GetProxy");
   pragma Import(MSIL,IsBypassed,"IsBypassed");
   pragma Import(MSIL,set_Credentials,"set_Credentials");
end MSSyst.Net.IWebProxy;
pragma Import(MSIL,MSSyst.Net.IWebProxy,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System]System.Net.IWebProxy");
