-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
limited with MSSyst.Drawing.Point;
with MSSyst.EventArgs;
limited with MSSyst.String;
limited with MSSyst.Type_k;
limited with MSSyst.Windows.Forms.MouseButtons;
with MSSyst.Windows.Forms.MouseEventArgs;
limited with MSSyst.Windows.Forms.TreeNode;
package MSSyst.Windows.Forms.TreeNodeMouseClickEventArgs is
   type Typ is new MSSyst.Windows.Forms.MouseEventArgs.Typ   with record
      null;
   end record;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   function new_TreeNodeMouseClickEventArgs(
      This : Ref := null;
      node : access MSSyst.Windows.Forms.TreeNode.Typ'Class;
      button : MSSyst.Windows.Forms.MouseButtons.Valuetype;
      clicks : Integer;
      x : Integer;
      y : Integer) return Ref;
   function get_Node(
      This : access Typ) return access MSSyst.Windows.Forms.TreeNode.Typ'Class;
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_TreeNodeMouseClickEventArgs);
   pragma Import(MSIL,get_Node,"get_Node");
end MSSyst.Windows.Forms.TreeNodeMouseClickEventArgs;
pragma Import(MSIL,MSSyst.Windows.Forms.TreeNodeMouseClickEventArgs,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System.Windows.Forms]System.Windows.Forms.TreeNodeMouseClickEventArgs");
