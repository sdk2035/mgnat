-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
limited with MSSyst.Runtime.Serialization.SerializationInfo;
limited with MSSyst.Runtime.Serialization.StreamingContext;
limited with MSSyst.String;
limited with MSSyst.Type_k;
with MSSyst.ValueType;
package MSSyst.RuntimeMethodHandle is
   type ValueType is new MSSyst.ValueType.Typ with null record;
   type ValueType_Arr is array (Natural range <>) of ValueType; -- start at 0
   type ValueType_Array is access all ValueType_Arr;
   type ValueType_addrof is access all ValueType;
   type ValueType_array_addrof is access all ValueType_Array;
   function Equals(
      This : Valuetype;
      obj : access MSSyst.Object.Typ'Class) return Standard.Boolean;
   function Equals(
      This : Valuetype;
      handle : MSSyst.RuntimeMethodHandle.Valuetype) return Standard.Boolean;
   function get_Value(
      This : Valuetype) return MSIL_Types.native_int;
   function GetFunctionPointer(
      This : Valuetype) return MSIL_Types.native_int;
   function GetHashCode(
      This : Valuetype) return Integer;
   procedure GetObjectData(
      This : Valuetype;
      info : access MSSyst.Runtime.Serialization.SerializationInfo.Typ'Class;
      context : MSSyst.Runtime.Serialization.StreamingContext.Valuetype);
   function op_Equality(
      left : MSSyst.RuntimeMethodHandle.Valuetype;
      right : MSSyst.RuntimeMethodHandle.Valuetype) return Standard.Boolean;
   function op_Inequality(
      left : MSSyst.RuntimeMethodHandle.Valuetype;
      right : MSSyst.RuntimeMethodHandle.Valuetype) return Standard.Boolean;
private
   pragma Import(MSIL,Equals,"Equals");
   pragma Import(MSIL,get_Value,"get_Value");
   pragma Import(MSIL,GetFunctionPointer,"GetFunctionPointer");
   pragma Import(MSIL,GetHashCode,"GetHashCode");
   pragma Import(MSIL,GetObjectData,"GetObjectData");
   pragma Import(MSIL,op_Equality,"op_Equality");
   pragma Import(MSIL,op_Inequality,"op_Inequality");
end MSSyst.RuntimeMethodHandle;
pragma Import(MSIL,MSSyst.RuntimeMethodHandle,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[mscorlib]System.RuntimeMethodHandle");
