...
procedure Gnatlink is
   Object_Suffix : String_Access := new String'(
   ...
   begin
      ...
      --    -gnatA   stops reading gnat.adc, since we don't know what options
      --             would work, and we do not need it anyway.
         ...
         begin
            ...
            --  Here if argument is a file name rather than a switch
               ...
               then
                 ...
                 and then Ali_Filename = null
               then
                  Ali_Filename := new String'(Arg & ".ali");

               elsif Arg'Length > Object_Suffix.all'Length
                 and then Arg
                   (Arg'Last - Object_Suffix.all'Length + 1 .. Arg'Last)
                                                = Object_Suffix.all
               then
   ...
   procedure Process_Binder_File (Name : in String) is
      ...
      Object_List_File_Supported : Boolean := False;
      ...
      --  Object_File_Option_Ptr : chars_ptr := System.Null_Address;
      --  pragma Import (C, Object_File_Option_Ptr, "object_file_option");
      --  Pointer to a string representing the linker option which specifies
      --  the response file.

      Using_GNU_Linker : Boolean := False;
   ...
   begin
      ...
      loop
         ...
         if Ada_Bind_File then
            Next_Line (Next_Line'First .. Nlast - 8) :=
              Next_Line (Next_Line'First + 8 .. Nlast);
            Nlast := Nlast - 8;
         end if;
         ...
         Link_Bytes := Link_Bytes + Nlast - Next_Line'First;
      end loop;
      ...
      if Next_Line (Next_Line'First .. Nlast) /= End_Info then
         loop
            --  Add binder options only if not already set on the command
            --  line. This rule is a way to control the linker options
            --  order.

            if not Is_Option_Present
              (Next_Line (Next_Line'First .. Nlast))
            then
               Linker_Options.Increment_Last;
...
begin
   ...
   if Hostparm.Java_VM then
      Gcc := new String'("mgnat");
      Object_Suffix := new String'(".il");
      Ada_Bind_File := True;
      Begin_Info := "-- BEGIN Object file/option list";
      End_Info   := "-- END Object file/option list   ";
   end if;
   ...
   if Base_Name (Command_Name) = "gnatlink"
     and then Output_Filename.all = "test"
   then
      Error_Msg ("warning: executable name """ & Output_Filename.all
                   & """ may conflict with existing command");
   end if;
   ...
   Make_Binder_Filenames : declare
      ...
      function Get_Maximum_File_Name_Length return Integer is
      begin
         return 255;
      end Get_Maximum_File_Name_Length;
...
--   if not Hostparm.Java_VM then
   if Hostparm.Java_VM then
      ...
      begin
         --  remove the extraneous -L and -l flags generated automatically
         --  mcc: bug detected 1/7/03 (added reverse)
         --  rv: added removal of '-g' (ilasm already configured for debug info)
         for J in reverse Linker_Options.First .. Linker_Options.Last loop
            if Linker_Options.Table (J) (1 .. 2) = "-L"
         ...
         Call_Linker1 : declare
            Success  : Boolean;
            Args     : Argument_List (1 .. Num_Args + 5);
            Index    : Integer := Args'First;

         begin
            Args (Index) := new String'("/exe");
            Index := Index + 1;

            Args (Index) := new String'("/debug");
            Index := Index + 1;

            Args (Index) := new String'("/quiet");
            Index := Index + 1;

            Args (Index) := new String'("/output=" & Output_Filename.all);
            Index := Index + 1;
      ...
      begin
         ...
         for J in Linker_Options.First .. Linker_Options.Last loop
            ...
            if Linker_Options.Table (J) (1 .. 17) = "-Xlinker --stack="
              or else Linker_Options.Table (J) (1 .. 12) = "-Wl,--stack="
            then
               if Stack_Op then
                  Linker_Options.Table (J .. Linker_Options.Last - 1) :=
                    Linker_Options.Table (J + 1 .. Linker_Options.Last);
