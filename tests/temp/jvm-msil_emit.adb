...
package body JVM.MSIL_Emit is
   ...
   procedure Generate_Handler_Entries
     (File : File_Type; Method   : Method_Id);
   procedure Generate_External_Assemblies (File : in File_Type);
   ...
   procedure Add_Assembly (Name : in String) is
      Rbracket : Natural := Index (Name, "]");
      Pound    : Natural := Index (Name, "#");
   begin
   ...
   ----------------------------------
   procedure Generate_External_Assemblies (File : in File_Type) is
      Names  : String  := To_String (Assembly_Names);
   ...
   function Translate_File_Name (Name : Name_Id) return String is
      Input_Name : String := Name_String (Name);
   ...
   function External_Name (Name : Name_Id) return String is
      Result : String := Name_String (Name);
   begin
   ...
   begin
      ...
      if Pkg /= "" then
         declare
            Result : String := Check_Reserved (Name_String (Name (Class)));
         begin
         ...
         declare
            Result : String := External_Name (Name (Class));
         begin
            return Result;
         end;
      end if;
   ...
   begin
      if Params = Null_Local_Var or else not Is_Param (Params) then
         ...
         if Names then
            declare
               Rest : String := Params_String_Helper (
                  Next_Local_Var (Params), Names);
               This : String := Type_String (Type_Of (Params)) &
   ...
   procedure Emit_Field (File : File_Type; Field : Field_Id);
   procedure Emit_Method (File : File_Type; Method : Method_Id);
   procedure Emit_Code (File : File_Type; Method : Method_Id);
   procedure Emit_Code_Sequence
     (File : File_Type; Method : Method_Id; Method_Seq : Code_Sequence);
   procedure Emit_CP_Float (File : File_Type; Value : Ureal);

   procedure Emit_Field (File : File_Type; Field : Field_Id) is
   begin
   ...
   procedure Emit_CP_Float (File : File_Type; Value : Ureal) is
   ...
   is
      ...
      M : Method_Id;
   begin
      M := Method;  --  so GNAT doesn't whine
      while Instr /= Null_Instr loop
         ...
         case MSIL_Instr.Op is
            ...
            when LDC_R8 =>
               Put (File, " ");
               Emit_CP_Float (File, Pool_Double (MSIL_Instr.Pool_Item));
   ...
   procedure Emit_Method (File : File_Type; Method : Method_Id) is
   ...
   begin
      ...
      Put_Line (File, Params_String (Method) & " { ");
   ...
   begin
      ...
      if not Opt.GNAT_Mode then
         ...
         Put_Line (File,
             "      DB E3 8D 03 7C 12 B3 CA F3 4F 5A AD B6 85 BC 0F ");
         Put_Line (File,
             "      41 91 1D 45 0A 03 19 AD BE 89 71 D9 1A 8C F7 F4 ");
         Put_Line (File,
             "      45 7A 6C 6A 13 77 59 B1 E5 BB 65 F6 03 98 B8 97 ");
         Put_Line (File,
             "      AB 49 B6 E0 52 EA E7 45 89 69 97 94 25 13 E9 4D ");
         Put_Line (File,
             "      CA 52 B5 ED 0E 37 DB 12 B0 B6 EE 43 CB AE 6C 38 ");
         Put_Line (File,
             "      99 42 07 69 97 F3 17 FA D8 29 1C 49 FA 6E 62 83 ");
         Put_Line (File,
             "      C9 05 0D 53 2E D9 E0 94 2D 68 DB B1 5E 22 68 52 ");
         Put_Line (File,
             "      14 D0 A6 A4 E9 52 C3 C1 DA 42 72 7F 22 B5 12 C0 )");
         Put_Line (File,
             ".ver 0:0:0:0");
         Put_Line (File, " }");
      end if;
...
--         ".ver 1:0:3300:0 }");
      ...
      begin
         if Interface_Ref /= Null_Entity_Ref then
            ...
            Put (File, External_Name (Interface (Interface_Ref)));
            ...
            loop
               ...
               Put_Line (File, ",");
               Put (File, "         " &
                     External_Name (Interface (Interface_Ref)));
            end loop;
            New_Line (File);
         end if;
      end;
