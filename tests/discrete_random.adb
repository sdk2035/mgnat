...
package body Discrete_Random is
   ...
   begin
      Put_Line("start of reset");
      put_line("year" & integer'image(integer(calendar.year(now))));
      put_line("month" & integer'image(integer(calendar.month(now))));
      put_line("day" & integer'image(integer(calendar.day(now))));
      X1 := Int (Calendar.Year    (Now)) * 12 * 31 +
            Int (Calendar.Month   (Now) * 31)     +
            Int (Calendar.Day     (Now));
      Put_Line("past x1");
      Put_Line("seconds" & Integer'Image(Integer(Calendar.Seconds(Now))));
      put_line("bob1");
      X2 := Int (Calendar.Seconds (Now) * Duration (1000.0));
      put_line("bob2");

      Put_Line(Integer'Image(Integer(X1)));
      Put_Line(Integer'Image(Integer(X2)));
