...
package body Jx_Decl is
      ...
      if Is_Overloadable (E) and then Has_Homonym (E) then
         ...
         while Present (Hmnym) loop
            if Scope (E) = Scope (Hmnym) then
               --  mcc: 08/31/05, can we do this to limit
               --  extraneous homonym counting?
               --  if not Is_Dispatching_Operation (E) or else
               if ((Ekind (E) = Ekind (Hmnym)) or
                   (Is_Subprogram (E) and Is_Subprogram (Hmnym)))
                  and then
                     DTC_Entity (E) = DTC_Entity (Hmnym) then
                  H_Num := H_Num + 1;
               end if;
            end if;
   ...
   function Undot (Name : in String) return String;
   --  mcc: replace '.' with '_' in a name
   function Undot (Name : in String) return String is
      Result : String := Name;
   begin
   ...
   begin
      ...
      --  mcc: but make sure this doesn't contain '.'
      --
   ...
   function Wrapped_Type (Obj_Or_Type : Type_Id) return Boolean is
   begin
      if Obj_Or_Type = Type_Of (API_Class (Ada_Int)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_Lng)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_UInt)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_ULng)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_Flt)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_Dbl)) or
         Obj_Or_Type = Type_Of (API_Class (Ada_Acc)) then
         return True;
      else
         return False;
      end if;
...
--         when E_Array_Type | E_String_Type | E_Enum_Table_Type =>
         ...
         when E_Task_Type | E_Protected_Type =>
            ...
            if JVM_Entity (Typ) = Null_Type then
               Set_Map (Typ, JVM_Type (Corresponding_Record_Type
                  (Typ)));
            end if;
   ...
   begin
      case Ekind (Disc_Type) is
         when E_Enumeration_Type =>
            --  mcc: in MSIL, we have enumeration types which are
            --  imported in a special way, so if we see the convention
            --  Java, we'll generate a new type.  Ada enumerations
            --  are just done as integers and could not be readily
            --  exported to other languages.  Fortunately, System.Enum
            --  doesn't look particularly useful
            if Convention (Disc_Type) /= Convention_Java then
         ...
         when E_Signed_Integer_Type | E_Modular_Integer_Type =>
            ...
            --  mcc: changed for MSIL, now is native int
            --  (or not, blew up the runtime!!!)
...
--               Set_Map (Disc_Type, Native_Int_Type);
              ...
              and then Name_String (Chars (Scope (Disc_Type))) = "msil_types"
            then
               Set_Map (Disc_Type, Native_Int_Type);
   ...
   procedure Declare_Record_Class (Rec_Type : Entity_Id) is
      ...
      pragma Warnings (Off, Deep_Clone);
      Target      : Local_Var_Id;
      ...
      Abst_Class  : constant Boolean  := Is_Abstract (Rec_Type);
   ...
   begin
--      if Name_String (Chars (Rec_Type)) = "valuetype" then
--         Set_Map (Rec_Type, New_Enum_Type (Rec_Type));
--         Set_Ekind (Rec_Type, E_Enumeration_Type);
--         Set_Suppress_Init_Proc (Rec_Type, True);
--         return;
--      end if;
      ...
      --  mcc: Ada 2005 now allows better syntax for this
      if Einfo.Is_Interface (Rec_Type) then
         Is_Intface := True;
      elsif Is_Tagged_Type (Rec_Type)
      ...
      if Is_Imported (Scope (Rec_Type)) then
         ...
         Rec_Class
           := New_Class (Name     => Class_Name (Rec_Type),
                         ...
                         Super    => Superclass,
                         Abstrct  => Abst_Class);
         --  mcc: I commented this out because it didn't seem
         --  appropriate for MSIL.
      ...
      if not Einfo.Is_Interface (Rec_Type)
        and then Convention (Rec_Type) /= Convention_Java
        and then Convention (Scope (Rec_Type)) /= Convention_Java
      then
   ...
   procedure Declare_Array_Type (Arr_Type : Entity_Id) is
      ...
      pragma Warnings (Off, Trg_Start);
      Source     : Local_Var_Id;
      pragma Warnings (Off, Source);
      Src_Count  : Local_Var_Id;
      pragma Warnings (Off, Src_Count);
      Src_Start  : Local_Var_Id;
      pragma Warnings (Off, Src_Start);
...
--         := Name (Name_String (Chars (Component_Type (Arr_Type))) & "[]");
   ...
   procedure Declare_Access_Type (Acc_Type : Entity_Id) is
      ...
      pragma Warnings (Off, Arr_Ref_Field);
      Dimensions       : Pos;
   ...
   begin
      ...
      then
         ...
         if Ekind (Designated_Atype) in Wrappable_Kind then
            --  mcc:  in .NET we are getting type info out for Int
            --  on with type .Ref's (which is bad), so I am going to
            --  patch it here
            if Ekind (Acc_Type) in Access_Kind and then
               From_With_Type (Acc_Type) and then
               Ekind (Designated_Atype) = E_Signed_Integer_Type then
               Error_Msg_Node_1 := Acc_Type;
               Error_Msg ("need to with package for type &",
                  Get_Current_Source_Loc);
               Error_Msg_Sloc := Sloc (Acc_Type);
               Error_Msg ("\defined at #",
                  Get_Current_Source_Loc);
            else
   ...
   begin
      ...
      while Present (Formal) loop
         ...
         --  mcc:  just added this on 15 Jan 2003
   ...
   procedure Declare_Package_Class (Pkg_Spec : Entity_Id) is
      ...
      function Generic_Class_Name (
         Node   : Entity_Id;
         Parent : String) return Name_Id is
         use Ada.Strings.Unbounded;
         Result : Unbounded_String := To_Unbounded_String (Parent & "<");
         Location : constant Natural :=
            Ada.Strings.Fixed.Index (Parent, "`");
         Count  : Natural;
         Parameter : Entity_Id := First_Entity (Pkg_Spec);
      begin
         --  the characters after the ` can be used to tell me
         --  how many generic type parameters there are
         ...
         for i in 1 .. Count - 1 loop
            Result := Result & JVM.MSIL_Emit.Type_String (JVM_Type (
               Etype (Parameter)))
               & ",";
            Set_Map (Parameter, JVM.Generic_Types (i - 1));
   ...
   begin
      ...
      if Present (Interface_Name (Pkg_Spec)) then
         if Present (Parent (Pkg_Spec)) and then
            Nkind (Parent (Pkg_Spec)) = N_Package_Specification and then
            Is_Imported (Pkg_Spec) and then
            Is_Generic_Instance (Pkg_Spec) then
            Get_Interface_Names (Interface_Name (Pkg_Spec),
               Pkg_Name, Class_Name);
            Class_Name := Generic_Class_Name (
               Pkg_Spec,
               Name_String (Class_Name));
         else
            Get_Interface_Names (Interface_Name (Pkg_Spec),
               Pkg_Name, Class_Name);
         end if;
         ...
         --  mcc:  In case this package is going to have a child package, we
         --  will append "_pkg" to the class name
         --  mcc: append _pkg only if not a procedure or function
         if Kind_Of /= E_Function and Kind_Of /= E_Procedure then
   ...
   procedure Declare_Method (Class : Class_Id; Subp : Entity_Id) is
      ...
      pragma Warnings (Off, AR_Param);
      Ftyp              : Type_Id;
      Method_Name       : Name_Id   := Name (Undot (
         Name_String (JVM_Entity_Name (Subp))));
      Class_Name        : Name_Id;
      Package_Name      : String_Id;
      ...
      pragma Warnings (Off, Disp_Formal);
      ND_Method         : Method_Id := Null_Method;
      ND_Formal         : Local_Var_Id;

      function Has_DT_Position (Typ : Entity_Id; Pos : Uint) return Boolean;
      --  does this type have this DT Position?
      ...
      function Extract_Name (Subp : in String) return String;
      --  get the subprogram name from [library]name

      function Extract_Name (Subp : in String) return String is
         Loc : constant Natural := Index (Subp, "]");
      begin
         return Subp (Loc + 1 .. Subp'Last);
      end Extract_Name;

      function Has_DT_Position (Typ : Entity_Id; Pos : Uint) return Boolean is
         Prim_Op : Elmt_Id := First_Elmt (Primitive_Operations (Typ));
      begin
         while Present (Prim_Op) loop
            if DT_Position (Node (Prim_Op)) = Pos then
               return True;
            end if;
            Next_Elmt (Prim_Op);
         end loop;
      ...
      begin
         ...
         --  mcc: This code used to do the above, but because we
         ...
         --    and then Has_DT_Entry (First_Tag_Component (Parent_Type))
           and then Has_DT_Position (Parent_Type, Op_Position)
         loop
   ...
   begin
      ...
      if Is_Constructor (Subp) then
         ...
         pragma Assert (Ekind (Subp) = E_Function
           and then
           (Name_String (Chars (Etype (Subp))) = "valuetype"
           or else
         ...
         --  Otherwise we create a new method and check for a 'this' formal
            ...
            Method := New_Method (Constructor_Class, J_String.Name (".ctor"),
      ...
      then
         ...
         Method := New_Method (Method_Class,
                               Method_Name,
                               JVM_Type (Subp),
                               Static => True,
      ...
      --  imported from Java).
         ...
         if Is_Dispatching_Operation (Subp) then
            ...
            --  mcc: the DT_Position thing here kills us
            --  because it sets back to non-homonym'd name
            --  so what I did is make sure the DT position doesn't
            --  fix the values for "=" (for one) in exp_util.adb
            if Controlling_Type /= Parent_Ctrl_Type then
               --  Has_Homonym (Subp) then
               declare
                  Prim_Op : Elmt_Id
         ...
         begin
            --  mcc:  attempting to make up for valuetype returns here
            --  it's not really dispatching if it returns a valuetype
            if not Make_Static and
                  Convention (Subp) = Convention_Java and
                  Ekind (Subp) = E_Function and
                  Has_Controlling_Result (Subp) then
               Make_Static := True;
            end if;
            Method := New_Method
                     (Method_Class,
      ...
      while Present (Formal) and then Ekind (Formal) in Formal_Kind loop
         ...
         --  mcc: System.Address maps to native int
         --  on C imports
         if Convention (Subp) = Convention_C and then
            Name_String (Chars (Etype (Formal))) = "address" then
   ...
   begin
      ...
      then
         ...
         --  mcc: 10/9/04 (fix bug on renaming exceptions)
         --  don't reset the entity if it already exists
