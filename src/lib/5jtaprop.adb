...
--   function To_Ptr is new Unchecked_Conversion (System.Address, Ptr);
   ...
   function ObjectToThread is new Unchecked_Conversion (Lang.Object.Ref,
      Lang.Thread.Ref);
   function ToThread (This : access System.OS_Interface.Thread)
      return Lang.Thread.Ref renames System.OS_Interface.Get_Thread;
   ...
   is
      --  mcc: debug
      --  procedure Debug (MS : in long; NS : in int);
      --  pragma Import (C, debug, "debug_split");
   begin
   ...
   function Monotonic_Clock return Duration is
      ...
      Millisecs : Interfaces.Java.long := Current_Time_100ns / 10_000;
   begin
   ...
   begin
      ...
      --  mcc 12/09/03: commented out
      --                b/c invalid cast error
      if Prio <= Last then
         Lang.Thread.Set_Priority
            (ObjectToThread (T.Common.LL.Thread),
            Lang.Priority.ValueType'Val (Lang.Priority.ValueType'Pos (
                 Lang.Priority.Lowest) +
                 (Lang.Priority.ValueType'Pos (Lang.Priority.Highest)
                 - 1 - Lang.Priority.ValueType'Pos (
                 Lang.Priority.Lowest)) *
                 (Prio - First) / (Last - First)));
      else
         --  Interrupt_Priority
         Lang.Thread.Set_Priority (ObjectToThread (T.Common.LL.Thread),
            Lang.Priority.Highest);
      end if;
   end Set_Priority;
   ...
   begin
...
#if not COMPACT'Defined
      Set_Daemon (ToThread (Thread), True);
#end if;
      --  Start the newly created thread

      Start (ToThread (Thread));
      ...
      if Priority <= Last then
         ...
         --  Interrupt_Priority
         Set_Priority (ToThread (Thread), Lang.Priority.Highest);
      end if;
   ...
   begin
      ...
      then
         ...
         if Self_ID.Common.Base_Priority <
            Lang.Priority.ValueType'Pos (Lang.Priority.Lowest) then
            Set_Priority (ObjectToThread (Self_ID.Common.LL.Thread),
               Lang.Priority.Lowest);
         elsif Self_ID.Common.Base_Priority >
            Lang.Priority.ValueType'Pos (Lang.Priority.Highest) then
            Set_Priority (ObjectToThread (Self_ID.Common.LL.Thread),
               Lang.Priority.Highest);
         else
            Set_Priority (ObjectToThread (Self_ID.Common.LL.Thread),
              Lang.Priority.ValueType'Val (Self_ID.Common.Base_Priority));
         end if;
      end if;

      Ignore := Wait (Self_ID.Common.LL.L);
   end Sleep;
   ...
   is
      ...
      Ignore      : Boolean;
      --  mcc: debug
      --  procedure Debug_Sleep (X, Y : in Duration);
      --  pragma Import (C, Debug_Sleep, "Debug_Sleep");
   begin
      pragma Assert (Self_ID = Self);
      ...
      --  Debug_Sleep (Abs_Time, Check_Time);
      if Abs_Time > Check_Time then
         if not (Self_ID.Pending_ATC_Level < Self_ID.ATC_Nesting_Level
            ...
            Ignore := Wait (Self_ID.Common.LL.L, int (Request_MS));
