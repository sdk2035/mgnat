...
private
   ...
   protected type Event_State is
      ...
      entry Set (Timeout : in Time;  Handler : Timing_Event_Handler);
      ...
      procedure Clear;
      --  Indicates that Set (for Set_Handler) can now change the timeout
      --  and/or handler. Resets the timeout to Time_First and the handler to
      --  null. Called only by the interal Timer task.

      entry Cancellation;
      --  Indicates that the timing event has been canceled by the user. Called
      --  only by the interal Timer task.

      procedure Cancel;
      --  Cancels a pending timing event timeout and enables the Cancellation
      --  entry. Called by procedure Cancel_Handler.
   ...
   private
      ...
      --  A flag controlling when users can change the Timeout and/or Handler
      --  values. In particular the entry Set, called by procedure Set_Handler,
      --  is controlled by this flag.

      Cancelled : Boolean := False;
      --  A flag controlling the Cancellation entry called by the timer task.
      --  The flag is set by protected procedure Cancel that is itself called
      --  by the application via procedure Cancel_Handler.
