...
package body Jx_Ch3 is
   ...
   begin
      ...
      --  mcc: we want to allow this for valuetypes in MSIL

      if Is_Tagged_Type (Repn_Type)
         and then Convention (Repn_Type) = Convention_Java
         and then Name_String (Chars (Repn_Type)) /= "valuetype"
      then
         Error_Msg_N
      ...
      if Present (Address_Clause (Obj_Entity)) then
         ...
         case Ekind (Repn_Type) is
            when E_Record_Type | E_Task_Type | E_Protected_Type =>
               ...
               and then Name_String (Name (Superclass (Class_Of_Type (
                  JVM_Type (Obj_Entity))))) /= "ValueType"
               then
                  ...
                  Gen_Invoke_Init (Class_Of_Type (JVM_Type (Obj_Entity)),
                     Repn_Type);
            ...
            when Wrappable_Kind =>
               ...
               --  potentially uninitialized).
                  ...
                  Gen_Store_Local (Local_Var);
               --  mcc: 05/15/06 resolve bug where wrapped types aren't
               --  getting initialized
               elsif Wrapped_Type (JVM_Type (Obj_Entity)) then

                  Gen_Default_Object (Class_Of_Type (
                     JVM_Type (Obj_Entity)));
      ...
      if Present (Init_Expr) and then not No_Initialization (Obj) then
         case Ekind (Repn_Type) is
            when Elementary_Kind =>
               ...
               elsif not Is_Global and then
                     Wrapped_Type (Type_Of (Local_Var)) then
                  Gen_Load_Local (Local_Var);

                  Evaluate_Expr (Init_Expr, Check_Subtype => Repn_Subt);

                  Gen_Put_Object_Field (
                     Wrapper_Field (Type_Of (Local_Var)));
               else
                  Evaluate_Expr (Init_Expr, Check_Subtype => Repn_Subt);
...
--         when E_Array_Type | E_String_Type | E_Enum_Table_Type =>
   ...
   begin
      ...
      case Ekind (Comp_Type) is
         when E_Record_Type | E_Task_Type | E_Protected_Type =>
            ...
            Gen_Invoke_Init (Class_Of_Type (JVM_Type (Comp_Subt)),
               Comp_Type);
   ...
   is
      ...
      begin
         case Ekind (Full_Comp_Type) is
            when E_Record_Type | E_Task_Type | E_Protected_Type =>
               ...
               Gen_Default_Object (Class_Of_Type (JVM_Type (Comp_Type)));
               ...
               if Present (Base_Init_Proc (Full_Comp_Type)) then
                  declare
                     Comp_LV : constant Local_Var_Id
                       := New_Local_Var ("_rec_cmp", JVM_Type (Comp_Type));
            ...
            when E_Array_Type | E_String_Type =>
               ...
               begin
                  ...
                  --  mcc: don't do this for valuetypes
      ...
      --  mcc: 10/13/05
      --  changed to create new array types so this will verify
      procedure Traverse_Subarrays (
            Dimensions   : Pos_8;
            Arr_JVM_Type : Type_Id) is
         Arr_Temp  : constant Local_Var_Id
           := New_Local_Var ("_arr_tmp", Arr_JVM_Type);
      ...
      begin
         ...
         --  mcc: 10/13/05 well, I shouldn't need to do this
         --  but it is rather expecting the types to all be the same
         ...
         --  a traversal of the subarrays of the next dimension.
            ...
            Traverse_Subarrays (Dimensions - 1,
               New_Array_Type (Element_Type (Arr_JVM_Type),
                  Dimensions - 1));
         end if;
   ...
   begin
      ...
      --  mcc: don't do anything for valuetype components
      if Name_String (Chars (Comp_Type)) = "valuetype" then
         return;
      end if;
                     ...
                     then
                        ...
                        --  lengths, which is a real pain! ???
                           ...
                           for D in 2 .. Number_Dimensions (Comp_Type) loop
                              --  mcc: 10/13/05 well, the array type
                              --  here needs to be generated and
                              --  gets smaller each time around the loop
                              --  for peverify
                              Pop_Type;
   ...
   procedure Generate_Discrete_Type (Discr_Type : Entity_Id) is
      pragma Warnings (Off, Discr_Type);
   begin
...
--   end case;
   ...
   begin
      ...
      --  mcc: need to add code here that collects the Ada 2005
      --  style interfaces for the type

      if Is_Tagged_Type (R_Type) then
         Interface_List := Abstract_Interfaces (R_Type);
         if Present (Interface_List) then
            Intface := First_Elmt (Interface_List);
            while Intface /= No_Elmt loop
               Associate_Interface (Record_Class, JVM_Class (
                  Node (Intface)));
               Next_Elmt (Intface);
            end loop;
         end if;
      ...
      --  mcc: Ada 2005 allows interfaces directly
      if not Einfo.Is_Interface (R_Type)
        and then Convention (R_Type) /= Convention_Java
   ...
   procedure Generate_Array_Type (A_Type : Entity_Id) is
      ...
      --  mcc: 10/13/05
      --  now create subtypes in recursion so this will verify
      procedure Assign_Subarrays (Dimensions : Pos_8;
         J_Nested_Type : Type_Id);
      ...
      begin
         ...
         then
            ...
            --  mcc: 10/13/05, again for peverify
            Pop_Type;
         ...
         --  generate a traversal of the subarrays of the next dimension.
            ...
            begin
               --  mcc: 10/13/05
               --  these are rather bad, but allow
               --  the types to match for peverify
               Pop_Type (2);
               Push_Type (J_Nested_Type);
               Push_Type (J_Nested_Type);
               ...
               if Dimensions = 1 then
                  ...
                  Assign_Subarrays (Dimensions - 1,
                     New_Array_Type (Element_Type (J_Nested_Type),
                        Dimensions - 1));
               end if;
   ...
   begin
      ...
      if Is_Tagged_Type (Comp_Type)
         ...
         and then Name_String (Chars (Scope (Parent_Subtype (Comp_Type)))) /=
            "valuetype"
      then
         Error_Msg_N
   ...
   begin
      ...
      then
         declare
            Access_Class : constant Class_Id :=
   ...
   procedure Generate_Subprogram_Access_Type (Subp_Acc_Type : Entity_Id) is
      Subp_Acc_Class : constant Class_Id  :=
         Class_Of_Type (JVM_Type (Subp_Acc_Type));
