...
with Hostparm; use Hostparm;  --  mcc:
...
package body Exp_Disp is
   ...
   Max_Prim_Op_Position : constant := 15;
   ...
   begin
      ...
      --  mcc: 02/14/06 just do a conversion??
   ...
   begin
      ...
      --  in Ada.Tags).
         ...
         while Present (Prim_Elmt) loop
            ...
            --  mcc: I don't trust this thing. (10/14/05)
            --  maybe I should
            if not Java_VM
         ...
         declare
            --   mcc: 02/14/06 hack away!
            --   just leave blank entries in the "table" for
            --   all defaults
            Fixed_Prim : array (Int range 0 .. Parent_EC + Count_Prim
               + Max_Prim_Op_Position)
               of Boolean := (0 .. Max_Prim_Op_Position => True,
                  others => False);
         ...
         begin
            ...
            while Present (Prim_Elmt) loop
               ...
               if Is_Predefined_Dispatching_Operation (Prim) then
                  ...
                  --  mcc: add fixed here??
                  --  02/14/06 not needed anymore as we set above
               ...
               --  mcc: 02/13/06 fix overriding?? added new elsif branch
               --  elsif Is_Overriding_Operation (Prim)
               --     and then Present (Overridden_Operation (Prim))
               --     and then DT_Position (Overridden_Operation (Prim))
               --        /= No_Uint
               --  then
               --     E := Overridden_Operation (Prim);
               --     Set_DT_Position (Prim, DT_Position (E));
               --     Fixed_Prim (UI_To_Int (DT_Position (E))) := True;
               end if;
