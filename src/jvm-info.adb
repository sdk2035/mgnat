...
package body JVM.Info is
   ...
   --  The properties of JVM type entities are encapsulated in
   --  type JVM_Type_Info.
   ...
   --  New_No_Entity         : JVM_Entity (No_Entity);
   ...
   begin
      ...
      JVM_Table.Table (Next_Entity_Index) := New_Class_Entity;
      return Class_Id (Next_Entity_Index);
   end New_Class;
   ...
   begin
      ...
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Class_Entity;
      JVM_Table.Table (Next_Entity_Index).Is_Interface := True;
      return Class_Id (Next_Entity_Index);
   end New_Interface;
   ...
   begin
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Field_Entity;
      return Field_Id (Next_Entity_Index);
   end New_Field;
   ...
   begin
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Method_Entity;
      return Method_Id (Next_Entity_Index);
   end New_Method;
   ...
   begin
      ...
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Type_Entity;
      Jtable (Next_Entity_Index).Type_Info := Init_Type;
      return Type_Id (Next_Entity_Index);
   end New_Type;
   ...
   begin
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Local_Var_Entity;
      return Local_Var_Id (Next_Entity_Index);
   end New_Local_Var;
   ...
   begin
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Label_Entity;

      Label_Counter := Label_Counter + 1;
      JVM_Table.Table (Next_Entity_Index).Label_Number := Label_Counter;

      return Label_Id (Next_Entity_Index);
   end New_Label;
   ...
   begin
      Next_Entity_Index := Next_Entity_Index + 1;
      JVM_Table.Set_Last (Next_Entity_Index);
      JVM_Table.Table (Next_Entity_Index) := New_Subroutine_Entity;
      return Subroutine_Id (Next_Entity_Index);
   end New_Subroutine;
   ...
   is
   begin
      return JVM_Entity_Ref (Jtable (I_Ref).Next_Entity_Ref);
   ...
   procedure Add_Interface (Class : Class_Id; I_Interface : Class_Id) is
      New_I_Ref : constant JVM_Entity_Ref := New_Entity_Ref;
      Intface_Ref : JVM_Entity_Ref;

   begin
      pragma Assert (Jtable (I_Interface).Kind = Class_Entity);
      pragma Assert (Jtable (I_Interface).Is_Interface);
      ...
      Intface_Ref := First_Interface_Ref (Class);

      while Intface_Ref /= Null_Entity_Ref loop
         if Name (Info.Get_Interface (Intface_Ref)) = Name (I_Interface)
           and then
             String_Equal
               (Pkg_Name (Info.Get_Interface (Intface_Ref)),
                Pkg_Name (I_Interface))
         then
            return;
         end if;

         Intface_Ref := Next_Interface_Ref (Intface_Ref);
      end loop;

      Jtable (New_I_Ref).Referenced_Entity := JVM_Id (I_Interface);
   ...
   begin
      case Type_Kind (Typ) is
         when Void_Kind =>
            --  mcc: 02/20/06 used by anonymous subprogram access?
            return 1;
            --  pragma Assert (False);
            --  raise Program_Error;
   ...
   begin
      pragma Assert (not Is_Open (Method) or else Is_Abstract (Method));
   ...
   begin
      ...
      Jtable (Method).Next_Index := Jtable (Method).Next_Index
         + 1;
--  mcc:  .NET doesn't do this, but just adds 1
--        + Local_Variable_Index (Word_Size (Jtable (Local).Variable_Type));
   ...
   procedure Set_Exception_Block (Method : Method_Id;
   ...
   procedure Set_Is_Open (Subr : Subroutine_Id; Value : Boolean := True) is
      ...
      --  mcc: !! (we should be setting to Value?!?!, but bug!)
   begin
      Jtable (Subr).Subr_Open := True;
   end Set_Is_Open;
...
begin
   ...
   New_Class_Entity.Name := No_Name;
   New_Field_Entity.Name := No_Name;
   New_Method_Entity.Name := No_Name;
   New_Type_Entity.Name := No_Name;
   New_Local_Var_Entity.Name := No_Name;
   New_Label_Entity.Name := No_Name;
   New_Subroutine_Entity.Name := No_Name;
   New_Entity_Reference.Name := No_Name;
