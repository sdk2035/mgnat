...
--  JVM.Info defines the JVM entities that serve as the JGNAT back end's
--  symbol table. The package exports operations for creating, updating,
--  and querying entities for the several forms of JVM 'id' types declared
--  in the specification of the JVM package, which correspond to classes,
--  fields, methods, types, local variables code labels, and
--  subroutines. Note that JVM.Info is currently defined as a private child
--  unit of JVM, and thus is available only to the implemention of the JVM
--  interface. The package may later be made public if we determine that
--  the upper levels of the back end require the means for direct access
--  and update of JVM entity information.
...
package JVM.Info is
   ...
   --  Returns an inner class's enclosing class; returns Null_Class
   --  if C does not denote an inner class.
   ...
   --  Returns the first of the list of references to interfaces that
   --  the class (or interface) implements; returns Null_Entity_Ref
   --  if there is no such interface.

   function Next_Interface_Ref (I_Ref : JVM_Entity_Ref) return JVM_Entity_Ref;
   --  Returns the next reference in a list of interface references;
   --  returns Null_Entity_Ref if I_Ref has no successor.
   ...
   --  raises an exception if I_Ref is null or if I_Ref does not refer to
   --  an interface entity.

   function First_Field (Class : Class_Id) return Field_Id;
   --  Returns the first in the list of fields of class C;
   --  returns Null_Field if the class has no fields.

   function First_Method (Class : Class_Id) return Method_Id;
   --  Returns the first in the list of methods of Class;
   --  returns Null_Method if the class has no methods.

   function First_Pool_Item (Class : Class_Id) return Pool_Id;
   --  Returns the first in the list of pool items of Class;
   --  returns Null_Pool_Item if the class has no pool items.
   ...
   --  Returns true if and only if Class has been opened via
   --  a call to Set_Is_Open (and has not yet been closed).
   ...
   --  Sets the outer class of Class to Parent. A call to this
   --  procedure has the effect of defining Class to be an inner class.

   procedure Set_Class_Type (Class : Class_Id; Class_Type : Type_Id);
   --  Associates Class with the given class type

   procedure Add_Interface (Class : Class_Id; I_Interface : Class_Id);
   --  Add an interface to the set of interfaces that class (or
   ...
   --  Adds a constant pool reference item to the list of constant pool
   --  items denoting references to Class.
   ...
   --  Returns the size allocated in words for variables and stack
   --  elements of the given type.

   function Element_Type (Array_Type : Type_Id) return Type_Id;
   --  Returns the element type of an array type; raises an
   --  exception if Array_Type does not denote an array type.

   function Dimensions (Array_Type : Type_Id) return Pos_8;
   --  Returns the number of dimensions of an array type; raises an
   --  exception if Array_Type does not denote an array type.
   ...
   --  Sets the element type of an array type; raises an exception
   --  if Array_Type does not denote an array type.

   procedure Set_Dimensions (Array_Type : Type_Id; Dimensions : Pos_8);
   --  Sets the number of dimensions of an array type; raises an
   --  exception if Array_Type does not denote an array type.

   procedure Set_Class (Class_Type : Type_Id; Class : Class_Id);
   --  Associates a class with its class type; raises an exception
   --  if Class_Type does not denote a class type.

   procedure Add_Pool_Ref (Typ : Type_Id; CP_Item : Pool_Id);
   --  Adds a constant pool reference item to the list of constant pool
   --  items denoting references to Typ.
   ...
   --  Returns a reference to the field that is the successor
   --  of Field; returns Null_Field if Field has no successor.
   ...
   --  Adds a constant pool reference item to the list of constant pool
   --  items denoting references to Field.
   ...
   --  Returns a reference to the method that is the successor
   --  of Method; returns Null_Method if Method has no successor.
   ...
   --  Returns the Method_Id of the parent method associated
   --  Method, else returns Null_Method if there is none.

   function First_Local_Var (Method : Method_Id) return Local_Var_Id;
   --  Returns the first local variable of the method; returns
   --  Null_Local_Var if the method has no local variables

   function Next_Local_Index (Method : Method_Id) return Local_Variable_Index;
   --  Returns the next available index for local variables of the
   --  method. The index of a method is initially 0, and the index
   --  gets incremented implicitly by calls to Add_Local_Var.
   ...
   --  Returns true if and only if Method has been opened via
   --  a call to Set_Is_Open (and has not yet been closed).

   function Is_Exception_Block (Method : Method_Id) return Boolean;
   --  Returns true if and only if Method is currently generating
   --  handled code

   function Is_Stack_Checking (Method : Method_Id) return Boolean;
   --  Returns true if and only if Method has operand stack checking
   --  enabled.
   ...
   --  Returns the first code label of the method; returns
   --  Null_Label if the method has no labels

   function First_Subroutine (Method : Method_Id) return Subroutine_Id;
   --  Returns the first subroutine of the method; returns
   --  Null_Subroutine if the method has no subroutines
   ...
   --  Returns the maximum word depth of the method's associated
   --  operand stack (only valid to call this after the method's
   --  code has been fully generated and the method is closed).
   ...
   --  increments the value of the method's local variable index
   --  according to the size of the variable.

   procedure Add_Pool_Ref (Method : Method_Id; CP_Item : Pool_Id);
   --  Adds a constant pool reference item to the list of constant pool
   --  items denoting references to Method.
   ...
   --  if Stack = Null_Op_Stack then any stack currently associated
   --  with the method will be freed.

   procedure Set_Max_Stack_Depth (Method : Method_Id; Depth : Depth_Range);
   --  Establishes the maximum word depth of the method's associated
   --  operand stack.
   ...
   --  Returns a reference to the local variable that is the successor
   --  of Local; returns Null_Local_Var if Local has no successor.
   ...
   --  Returns the label that is the successor of Label; returns
   --  Null_Label if Label has no successor.
   ...
   --  Returns true if and only if an instruction targeting the
   ...
   --  Returns a reference to the subroutine that is the successor
   --  of Subr; returns Null_Subroutine if Subr has no successor.
   ...
   --  Returns the maximum word depth of the subroutine's associated
   --  operand stack (only valid to call this after the subroutine's
   --  code has been fully generated and the subroutine is closed).

   function Is_Open (Subr : Subroutine_Id) return Boolean;
   --  Returns True if and only if the subroutine has not yet been
   --  closed via a call to Close_Subroutine.
   ...
   --  Associates Label with the subroutine (the label marks the entry
