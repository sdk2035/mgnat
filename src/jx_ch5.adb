...
package body Jx_Ch5 is
   ...
   procedure Assign_Multiarray
     (Target     : Node_Id;
      ...
      Dimensions : Pos_8)
   is
      pragma Warnings (Off, Dimensions);
   begin
   ...
   begin
      case Ekind (Full_Type (Target)) is
         when Elementary_Kind =>
            ...
            --  well, this makes me a little nervous, but it makes
            --  methods for valuetypes work correctly (as below)
            --  mcc: 11/6/2005 (test_task_string bug)
            if JVM_Type (Target) /= JVM_Type (Source) then
               ...
               begin
                  ...
                  if (Tk1 = Int_Kind or Tk1 = Long_Kind) and
                     (Tk2 = Int_Kind or Tk2 = Long_Kind) then
         ...
         when Einfo.Record_Kind =>
            ...
            if Name_String (Chars (Full_Type (Target))) = "valuetype" then
               Addr := Evaluate_Addr (Target);
               Evaluate_Expr (Source, Check_Subtype => Etype (Target));
               JVM.Pop_Type;
               Push_Type (JVM_Type (Target));
   ...
   begin
      ...
      --  mcc: need an exit label from the parent if this
      --  is the 2nd exit for the same parent loop
      else
         Exit_Label := JVM_Entity (Parent_Stmt);
      end if;
   ...
   begin
      ...
      --  mcc: clear any label if we've already done
      --  this loop (bug 20030120a)
      --  failed for more complicated, so must
      --  redo exception handling
      --  if Present (Identifier (Loop_Stmt)) then
      --     Loop_Id := Entity (Identifier (Loop_Stmt));
      --     if Present (Loop_Id) then
      --        Set_Map (Loop_Id, Null_Label);
      --     end if;
      --  end if;
      ...
      if Present (Iter_Scheme) then
         ...
         if Present (Param_Spec) then
            ...
            --  mcc: 20050820 somehow with GNAT05 we were
            --  getting 64 bit universal integers that
            --  needed to be converted to I4
            ...
            --                goto repeat_label
            --  mcc:  I instead want below so that I can't
            --        have the problem where it is impossible
            --        to increment past the end of the range
            --                load loop_index
            --                load loop_limit
            --                bgt  exit_label
