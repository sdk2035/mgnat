...
package body JVM.Emit is
   ...
   Short_Load_Ops : array (Load_Ops, Short_Local_Index) of Operation
   ...
   Short_Store_Ops : array (Store_Ops, Short_Local_Index) of Operation
   ...
   function Full_Class_Name (Class : Class_Id) return String;
   ...
   function Full_Class_Name (Class : Class_Id) return String is
   ...
   begin
      CP_Integer.Bytes := To_U4 (Int_32 (UI_To_Int (Int_Value)));
   ...
   function CP_Fieldref_Index (Field : Field_Id) return CP_Index_Field is
      CP_Id : constant Pool_Id := Field_Item (Gen_Class, Field);

   begin
      if Pool_Index (CP_Id) = CP_Empty then
         Emit_CP_Fieldref (CP_Id);
      end if;

      return Pool_Index (CP_Id);
   end CP_Fieldref_Index;

   ------------------------
   -- CP_Methodref_Index --
   ------------------------

   function CP_Methodref_Index (Method : Method_Id) return CP_Index_Method is
      CP_Id : constant Pool_Id := Method_Item (Gen_Class, Method);

   begin
      if Pool_Index (CP_Id) = CP_Empty then
         Emit_CP_Methodref (CP_Id);
      end if;

      return Pool_Index (CP_Id);
   end CP_Methodref_Index;
            ...
            Line.Add
              (Code.Line_Number_Table,
               Line_Info'(Start_PC => PC,
                          Line_Number =>
                            U2 (Sinput.Get_Physical_Line_Number
   ...
   is
      ...
      I_Size : U4 := Instr_Size (JVM_Instr, Code_Index);
         ...
         case Branch_Instr.Op is
            ...
            when Ifeq      | Ifne      | Iflt      | Ifge      | Ifgt | Ifle |
               ...
               declare
                  ...
                  Jump_Instr : Instr_Id := New_Instr;
                  Lbl_Instr  : Instr_Id := New_Instr;
                  Lbl_Id     : Label_Id := New_Label;
   ...
   begin
      case JVM_Instr.Op is
         ...
         when Sipush =>
            ...
            File_Instr.Short := Int_16 (JVM_Instr.Sint);
         ...
         when Iinc =>
            ...
            File_Instr.Const_Val := Int_16 (JVM_Instr.Increment);
            Set_Wide_Format_If_Needed (File_Instr);
         ...
         when Ldc_W =>
            ...
            pragma Assert (File_Instr.CP_Const_U2 in 1 .. 2**16 - 1);

         when Ldc2_W =>

            File_Instr.CP_Const := Pool_Index (JVM_Instr.Pool_Item);

            pragma Assert (File_Instr.CP_Const in 1 .. 2**16 - 1);
   ...
   begin
      ...
      begin
         ...
         while Interface_Ref /= Null_Entity_Ref loop
            ...
            Class_Index.Add
              (C_File.Interfaces, CP_Class_Index (Interface (Interface_Ref)));
      ...
      declare
         CF_Name   : constant String := Name_String (Name (Class)) & ".class";
