...
------------------------------------------------------------------------------
--  Modification log:
--  2006-Apr-17 (mcc) : fixed problem where all ranges too big
------------------------------------------------------------------------------
...
package body Jx_Swtch is
   ...
   is
      ...
      All_Empty   : Boolean;
      Others_Alt  : Boolean;
      pragma Warnings (Off, Others_Alt);
      Have_Too_Big_Alt  : Boolean;
      --  mcc:  Ok, so the problem described below is that you have really
      --  large switch tables being generated.  .NET doesn't have a lookup
      --  statement so that isn't a viable option.  What I'll choose to do
      --  instead is limit the size of the switch table to the threshold.
      --  The way that makes sense to me to do this is to remember the low
      --  and high so far, and if adding this alternative will make the range
      --  go out outside the threshold, then just do a Generate_Range_Test
      --  instead
      Range_Lowest   : Uint;
      Range_Highest  : Uint;
   ...
   begin
      ...
      while Present (Altern) loop
         ...
         while Present (Choice) loop
            ...
            elsif (Nkind (Choice) = N_Identifier or
              Nkind (Choice) = N_Expanded_Name)
