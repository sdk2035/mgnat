...
package body Exp_Ch5 is
   ...
   begin
      ...
      --  mcc: don't do all this funky expansion stuff for valuetypes
      elsif Is_Tagged_Type (Typ) and then
            Namet.Get_Name_String (Chars (Etype (Lhs))) = "valuetype" then
         ...
         begin
            ...
            if Is_Class_Wide_Type (Typ)
               ...
               or else (Is_Tagged_Type (Typ)
                          and then Namet.Get_Name_String (Chars (Etype (Lhs)))
                             /= "valuetype"
   ...
   function Make_Tag_Ctrl_Assignment (N : Node_Id) return List_Id is
      ...
      Res       : List_Id;
      Tag_Tmp   : Entity_Id;
      --  mcc: 02/13/06 these 3 readded from previous version
      Prev_Tmp : Entity_Id;
      Next_Tmp : Entity_Id;
      Ctrl_Ref : Node_Id;
   ...
   begin
      ...
      --  mcc: 02/13/06 the code from here to ***end*** has been
      --  copied from an older version, as the new slice
      --  mechanism just doesn't work for us
      ...
      --  mcc: 02/13/06 this code also readded from previous version
      --  end marked with ***end2***
