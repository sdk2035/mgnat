...
package body Exp_Ch3 is
   ...
   begin
      ...
      if Suppress_Init_Proc (A_Type) or else
         Name_String (Chars (Comp_Type)) = "valuetype" then
         return;
      end if;
   ...
   begin
      --  mcc: nothing to do if this is a valuetype
      if Chars (Typ) /= No_Name and then
         (Name_String (Chars (Typ)) = "valuetype" or else (
         Chars (Component_Type (Typ)) /= No_Name and then
         Name_String (Chars (Component_Type (Typ))) = "valuetype")) then
         return No_List;
      end if;
...
--                      Attribute_Name => Name_Unrestricted_Access));
      ...
      begin
         if Etype (Rec_Type) = Rec_Type
           ...
           or else not Present (Abstract_Interfaces (Rec_Type))
      ...
      begin
         ...
         then
            ...
            if not Is_CPP_Class (Etype (Rec_Type)) then
               ...
               --  mcc: 02/13/06 don't need for Java VM??
         ...
         --  mcc:  even though the init proc may be null at this time
         --  it might get some stuff added to it later by the MSIL
         --  backend, so don't let it not be called
         if List_Length (Body_Stmts) = 1
           and then Nkind (First (Body_Stmts)) = N_Null_Statement
           and then not Java_VM
         then
   ...
   begin
      ...
      --  mcc: don't do this for a valuetype
   ...
   begin
      ...
      if No (Expr) then
         ...
         --  mcc: don't do this for valuetype
         if Has_Non_Null_Base_Init_Proc (Typ)
           and then not No_Initialization (N)
           and then Namet.Get_Name_String (Chars (Typ)) /= "valuetype"
         then
   ...
   begin
      ...
      if Is_Tagged_Type (Def_Id) then
         ...
         if Is_CPP_Class (Def_Id) then
            ...
            --  mcc: 02/13/06 what is this thing??
            --  Append_Freeze_Actions
      ...
      --  mcc: don't need init for interfaces-- they're abstract!
      --  helps peverfify (10/14/05)
      if not Is_Interface (Def_Id) then
