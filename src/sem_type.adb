...
package body Sem_Type is
   ...
   begin
      ...
      if No (T1) or else No (T2) then
         if Total_Errors_Detected /= 0 then
            ...
            --  mcc: 02/19/06 this happens sometimes, why??
            --  raise Program_Error;
            return False;
         end if;
      end if;

      --  mcc:  we want native int to be compatible with system.address
      if Namet.Get_Name_String (Chars (T1)) = "address"
        and then Namet.Get_Name_String (Chars (Scope (T1))) = "system"
        and then Is_Library_Level_Entity (Scope (T1))
        and then (Namet.Get_Name_String (Chars (T2)) = "native_int"
            or Namet.Get_Name_String (Chars (T2)) = "unsigned_int8_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "unsigned_int16_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "unsigned_integer_addrof"
            or Namet.Get_Name_String (Chars (T2)) =
               "unsigned_long_long_integer_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "int8_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "int16_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "int32_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "int64_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "float32_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "float64_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "bool_addrof"
            or Namet.Get_Name_String (Chars (T2)) = "char_addrof")
        and then Namet.Get_Name_String (Chars (Scope (T2))) = "msil_types"
      then
         return True;
      end if;

      --  mcc: cont'd
      if Namet.Get_Name_String (Chars (T2)) = "address"
        and then Namet.Get_Name_String (Chars (Scope (T2))) = "system"
        and then Is_Library_Level_Entity (Scope (T2))
        and then (Namet.Get_Name_String (Chars (T1)) = "native_int"
            or Namet.Get_Name_String (Chars (T1)) = "unsigned_int8_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "unsigned_int16_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "unsigned_integer_addrof"
            or Namet.Get_Name_String (Chars (T1)) =
               "unsigned_long_long_integer_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "int8_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "int16_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "int32_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "int64_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "float32_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "float64_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "bool_addrof"
            or Namet.Get_Name_String (Chars (T1)) = "char_addrof")
        and then Namet.Get_Name_String (Chars (Scope (T1))) = "msil_types"
      then
         return True;
      end if;

      --  mcc: deleted else from 1st if as we can't get here o/w
      BT1 := Base_Type (T1);
      BT2 := Base_Type (T2);
   ...
   begin
      ...
      --  mcc: check for overloaded Java convention stuff
      --  because the MSIL libraries do sick things
      --  like Console.WriteLine where it matches
      --  two different overloads, so just pick the first
      if Ekind (Nam1) = Ekind (Nam2) and
         (Ekind (Nam1) = E_Procedure or
          Ekind (Nam1) = E_Function) and
         Convention (Nam1) = Convention (Nam2) and
         Convention (Nam1) = Convention_Java then
         return It2;
      end if;
