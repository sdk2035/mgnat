...
package body Jx_Drive is
   ...
   begin
      ...
      J_String_Type
        := New_Array_Type (JVM.Int_Type, 1,
           Name ("Standard.Wide_Wide_String"));
      Set_Type_Map (Standard_Wide_Wide_String, J_String_Type);
   ...
   begin
      ...
      case Nkind (Node) is
         --   mcc: this is a new kind of node I don't know about
         ...
         when N_Package_Declaration =>
            ...
            --  mcc: 05/17/06
            if Present (Generic_Parent (Specification (Node))) and then
               Convention (Generic_Parent (Specification (Node))) =
                  Convention_Java then
               null;
            else
         ...
         when N_Validate_Unchecked_Conversion =>
            ...
            begin
               ...
               then
                  null;
               --  mcc: 10/14/05, now that I've discovered ldind
               --  instructions, maybe I can do these
               elsif JVM.Type_Kind (J_Target) = JVM.Byte_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Char_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Short_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Int_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Long_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Float_Kind or
                  JVM.Type_Kind (J_Target) = JVM.Double_Kind
               then
                  null;
