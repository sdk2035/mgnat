...
package body Bindgen is
   ...
   begin
      --  Rvn: (checked by mcc: 10/09/05)
      --  Elaborate only once!
      WBI ("");
      WBI ("   Elab_Done : Boolean := False;");
      WBI ("");
         ...
         begin
            ...
            then
               ...
               if Hostparm.Java_VM then
                  ...
                  --  mcc: ugly hack, but we think only address_image
                  --  isn't a package
                  --  mcc: 02/09/06 we can ask!!
                  if U.Unit_Kind = 's' then
                  --  if Name_Buffer (1 .. Name_Len - 2) =
                  --     "system.address_image" then
                     Set_String (".");
                  else
                     Set_String ("_pkg.");
                  end if;
      ...
      --  file to deal with cross-library calls in some systems.
         ...
         --  mcc: 10/13/05 help make this pass peverify
         WBI ("         Restrictions             : String;");
         WBI ("         Interrupt_States         : String;");
         ...
         --  mcc: 10/13/05 help pass peverify
         WBI ("         Restrictions             => Restrictions,");
      ...
      for E in Elab_Order.First .. Elab_Order.Last loop
         ...
         begin
            ...
            --  since the assignment will be done when we process the body.
               ...
               --  mcc: 02/09/06 exp_attr isn't going to be smart enough
               --  to figure out whether or not to add _pkg, but we are!
               if Hostparm.Java_VM and then U.Unit_Kind = 'p' then
   ...
   begin
      ...
      if not Opt.No_Stdlib then
         ...
         --  mcc: remove for MGNAT
         if not Hostparm.Java_VM and then With_GNARL then
            Name_Len := 0;
   ...
   procedure Gen_Output_File (Filename : String) is
      --  mcc: change to false
      Is_Public_Version : constant Boolean := False;
      Is_GAP_Version    : constant Boolean := False;
