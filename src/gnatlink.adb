...
procedure Gnatlink is
   ...
   --  mcc: commented out import
   Object_List_File_Supported : constant Boolean := False;
   --  pragma Import
               ...
               --  mcc: add compact
                  ...
                  case Arg (2) is
                     ...
                     when 'o' =>
                        ...
                        if not Hostparm.Java_VM then
                           Output_File_Name := new String'(Argument (
                              Next_Arg));
                        else
                           Output_File_Name := new String'("/OUTPUT=" &
                              Argument (Next_Arg));
                        end if;
   ...
   procedure Process_Binder_File (Name : in String) is
      ...
      --  mcc: commented out pragma
      Link_Max : constant Integer := 1000;
      ...
      --  mcc: commented out pragma
      Run_Path_Option_Ptr : constant Interfaces.C.Strings.chars_ptr :=
      ...
      --  mcc: commented out pragma
      Object_Library_Ext_Ptr : constant Interfaces.C.Strings.chars_ptr :=
         Interfaces.C.Strings.Null_Ptr;
      --  pragma Import
      ...
      --  mcc: commented out
      Object_File_Option_Ptr : constant Interfaces.C.Strings.chars_ptr :=
         Interfaces.C.Strings.Null_Ptr;
      --  pragma Import (C, Object_File_Option_Ptr,
      --     "__gnat_object_file_option");
      --  Pointer to a string representing the linker option which specifies
      --  the response file.

      --  mcc: set to false
      Using_GNU_Linker : constant Boolean := False;
...
begin
   ...
   if Hostparm.Java_VM then
      ...
      begin
         ...
         --  rv: added removal of '-g'
         --      (ilasm already configured for debug info)
         --  mcc: added check for null params on 08/22/05
         --       as ilasm choked on it
