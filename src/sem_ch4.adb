   ...
   begin
      ...
      then
         ...
         while Present (Actual) and then Present (Formal) loop
            ...
            then
               --  mcc: here what we are doing is finding out if the formal
               --  is of type [mscorlib]System.String and the actual
               --  is of type Standard.String, in which case we will
               --  add a conversion automatically.  This perhaps defeats
               --  some of Ada's strong-typing, but makes the code a lot
               --  more readable for a novice programmer interfacing with
               --  the MS .NET Framework libraries
               --  if Is_String_Type (Etype (Actual)) and then
                  --  Is_Access_Type (Etype (Formal)) and then
                  --  Get_Name_String (Chars (Scope (Etype (Formal)))) =
                  --  "string"
               if (Is_String_Type (Etype (Actual)) and then
                  Is_Access_Type (Etype (Formal)) and then
                  Get_Name_String (Chars (Scope (Etype (Formal)))) =
                     "string") or else
                  (Is_String_Type (Etype (Actual)) and then
                  Ekind (Etype (Formal)) = E_Anonymous_Access_Type and then
                  Get_Name_String (Chars (Scope (
               ...
               then
                  declare
                     Converter : Node_Id;
                  begin
                     Converter := New_Copy (Actual);
                     --  mcc: fix bug 20050203 where the copy's kids
                     --  don't have their parent fields set properly
                     --  06/17/05
                     if Nkind (Converter) in N_Binary_Op then
                        Set_Parent (Left_Opnd (Converter), Converter);
                        Set_Parent (Right_Opnd (Converter), Converter);
                     elsif Nkind (Converter) in N_Unary_Op then
                        Set_Parent (Right_Opnd (Converter), Converter);
                     end if;
                     Change_Node (Actual, N_Op_Plus);
                     Set_Chars (Actual, Name_Op_Add);
                     Set_Parent (Converter, Actual);
                     Set_Right_Opnd (Actual, Converter);
                     Set_Etype (Actual, Etype (Formal));
                     Set_Etype (Converter, Standard_String);
                     Set_Analyzed (Actual, False);
                     Set_Analyzed (Converter, False);
                     Analyze (Actual);
                  end;
                  Next_Actual (Actual);
                  Next_Formal (Formal);

               elsif Has_Compatible_Type (Actual, Etype (Formal)) then
                  Next_Actual (Actual);
                  Next_Formal (Formal);
      ...
      if Etype (N) = Any_Type then
         ...
         begin
            ...
            if Nkind (N) in N_Unary_Op then
               ...
               --  mcc:  since we add op + automatically for
               --  mssyst.string.ref, users may not know they need
               --  the use clause to make it visible
               if Etype (R) = Standard_String then
                  Error_Msg_N ("\use clause of MsSyst.String will fix", N);
               end if;
               return;
