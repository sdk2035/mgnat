...
package body JVM.Code is
   ...
   procedure Print_Code_Sequence (Seq : in Code_Sequence) is
      Code : Instr_Id := Seq.First;
   begin
      while Code /= Null_Instr loop
         Put_Line (MSIL_Operation'Image (Code_Table.Table (Code).Op));
   ...
   function Stack_Height (Stack : Op_Stack_Id) return Natural is
   begin
   ...
   function Image (X : in MSIL_Operation) return String is
      function Keyword_Translate (X : in String) return String;

      function Keyword_Translate (X : in String) return String is
      begin
         if X (X'Last - 1 .. X'Last) = ".k" then
   ...
   begin
      return Keyword_Translate (Translate (
            Source  => To_Lower (MSIL_Operation'Image (X)),
