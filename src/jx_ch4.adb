...
package body Jx_Ch4 is
               ...
               case Ekind (Ada_Obj) is
                  when E_Variable | E_Constant =>
                     ...
                     then
                        ...
                        --  mcc: fix for indexed_component
                          or Nkind (Renamed_Object (Ada_Obj))
                          = N_Indexed_Component
                        then
                           Ada_Type
                        ...
                        --  mcc: 12/01/04 (primeserver.adb test)
                        elsif Nkind (Renamed_Object (Ada_Obj))
                     ...
                     then
                        ...
                        if Ekind (Ada_Type) not in Wrappable_Kind and then
                           Name_String (Chars (Ada_Type)) /= "valuetype" then
                     ...
                     then
                        --  mcc: 12/01/04 (primeserver.adb fix)
                        --  pragma Assert (Nkind (Renamed_Object (Ada_Obj))
                        --                  = N_Selected_Component);

                        if Nkind (Renamed_Object (Ada_Obj))
                           = N_Selected_Component then
                           declare
                           ...
                           N_Explicit_Dereference and then
                           Nkind (Prefix (Renamed_Object (Ada_Obj)))
                           = N_Selected_Component
                        then
                           declare
                              Designated_Atype : constant Entity_Id
                                 := Underlying_Type
                                   (Designated_Type (Full_Type (Prefix (
                                     Renamed_Object (Ada_Obj)))));
                              --  Designated_Jtype : constant Type_Id
                              --     := JVM_Type (Designated_Atype);

                           begin
                              Evaluate_Expr (Prefix (
                                 Renamed_Object (Ada_Obj)));

                              if Ekind (Designated_Atype) in
                                 Wrappable_Kind then
                                 Addr := (Field_Address,
                                    Wrapper_Field (Designated_Atype));
                  ...
                  when E_Procedure | E_Function =>
                     ...
                     begin
                        ...
                        then
                           ...
                           --  mcc: but for MSIL, we can load a function ptr
                           --  as a native int
                           else
   ...
   is
      -------------------------------------------------------
      --  (mcc)Fixup: replaces '-' with '_' in a filename  --
      --  as well as replacing '.' with '_'                --
      -------------------------------------------------------
      ...
      begin
         for I in Name'Range loop
            if Name (I) = '-' then
               Result (Result'First + I - Name'First) := '_';
            elsif Name (I) = '.' then
               Result (Result'First + I - Name'First) := '_';
            else
               Result (Result'First + I - Name'First) := Name (I);
            end if;
         end loop;
         return Result;
      end Fixup;
      ...
      AR_Field       : Field_Id;
      Count_String   : constant String := Natural'Image (Counter);
      Class_Name     : constant String
        := Fixup (Name_String (Name (Subp_Acc_Class))
         & "." & JVM_Expanded_Name (Subp_Entity) & "."
         & Name_String (Source_Name (Sloc (Subp_Name)))
         & Count_String (Count_String'First + 1 .. Count_String'Last));
   ...
   begin
      --  mcc: was having a problem where the same subprogram
      --  if 'access was used twice then I was getting two identical
      --  classes.  It would probably be ideal to just combine these
      --  into one, but it is easier to just uniquely name them
      Counter := Counter + 1;
      ...
      while Present (Formal) loop
         ...
         --  mcc:  just added this on 16 Jan 2003
      ...
      if Is_Dispatching_Operation (Subp_Entity) then
         ...
         if Present (Subp_Formal) then
            ...
            --  mcc: 10/13/05 needed for peverify!
      ...
      while Acc_Formal /= Null_Local_Var loop
         ...
         if not Present (Ctrl_Formal) or else Subp_Formal /= Ctrl_Formal then
            Gen_Load_Local (Acc_Formal);
            --  mcc:  just added this on 16 Jan 2003
            --  Load a pair of parameters for the lower and upper bounds of
            --  an array formal for each dimension of the array. Note that
            --  the bounds are suppressed if the subprogram is imported.
            ...
            then
               for Dimension in 1 .. 2*Number_Dimensions (
                  Etype (Subp_Formal)) loop
                  Acc_Formal  := Next_Local_Var (Acc_Formal);
                  Call_Formal := Next_Local_Var (Call_Formal);
                  Gen_Load_Local (Acc_Formal);
               end loop;
   ...
   begin
      case Attr_Kind is
         when Attribute_Tag =>
            Gen_Default_Object (JVM_Class (Prefix_Type));
            Gen_Invoke_API_Method (Name => Object_getClass);
            --  JVM.Gen_Push_String_Const (
            --     J_String.Name_String (Name (JVM_Class (Prefix_Type))));
         when Attribute_First | Attribute_Last =>
            ...
            then
               ...
               --  from the access value's referenced array descriptor.
                  ...
                  --  mcc: maybe this is an anonymous access type?
                  if Convention (Scope (Acc_Type)) = Convention_Java or
                     (Is_Local_Anonymous_Access (Acc_Type) and
                      Convention (Scope (Desig_Type)) = Convention_Java)
                  then
                     if Attr_Kind = Attribute_First then
         ...
         when Attribute_Pos =>
            ...
            if JVM_Type (Etype (Attr)) = ULong_Type
              and then Top_Type /= ULong_Type
            then
               Gen_Conversion (ULong_Type);
            end if;
         ...
         when Attribute_Size | Attribute_Max_Size_In_Storage_Elements =>
            ...
            then
               if JVM_Type (Etype (Attr)) = Int_Type then
                  Gen_Push_Int (RM_Size (Full_Subtype (Attr)));
               else
                  Gen_Push_Long (RM_Size (Full_Subtype (Attr)));
               end if;
         ...
         when Attribute_Access |
            ...
            --  In all other cases simply evaluate the prefix
               ...
               if Attr_Kind /= Attribute_Address then
                  ...
                  --  Mcc: for MSIL, load a function pointer for the
                  --  Invoke method
                  elsif Ekind (Prefix_Type) = E_Access_Subprogram_Type then
                     declare
                        Subp : Method_Id;
                     begin
                        Subp := JVM.Method (JVM_Class (Prefix_Type),
                           "Invoke");
                  ...
                  then
                     ...
                     Error_Msg_N
                       ("generating null address here, use aliased?",
                        Attr);

                  --  Otherwise it's safe to simply get the address
                  elsif Ekind (Prefix_Type) in Elementary_Kind
                    and then Is_Aliased_View (Attr_Prefix)
                  then
                     Addr := Evaluate_Addr (Attr_Prefix);
                     Gen_Get_Object_Field_Address (
                        Wrapper_Field (Prefix_Type));
                  else
                     Addr := Evaluate_Addr (Attr_Prefix);
                  end if;
               end if;
               ...
               --  mcc: on MSIL this is changed to Native Int
               if Attr_Kind = Attribute_Address
                 or else Attr_Kind = Attribute_Code_Address
               then
                  Pop_Type;
                  Push_Type (Native_Int_Type);
               end if;
            end if;
         --  mcc: just do nothing for alignment for now to avoid error
   ...
   begin
      ...
      if Lit_Length > 0 then
         ...
         then
           ...
           Root_Type (Component_Type (Str_Type)) = Standard_Wide_Wide_Character
         then
            Gen_Invoke_API_Method (String_getWideChars);
   ...
   begin
      ...
      elsif J_Type = Long_Type or J_Type = ULong_Type then
   ...
   is
      ...
      Formal_LV   : Local_Var_Id;
      Formal_Type : Type_Id;
      pragma Warnings (Off, Formal_Type);
      AR_Fld      : Field_Id;
   ...
   begin
      ...
      case Exp_Kind is
                     ...
                     then
                        ...
                        if Nkind (Renamed_Object (Ada_Entity))
                           ...
                           begin
                              ...
                              --  Change the Ada_Entity to the
                              --  selected component to allow handling
                              --  the case where it's wrapped.

                              Ada_Entity := Selector;
                           end;

                        --   mcc: 10/8/04
                        --   to fix "unsupported scalar renaming"
                        elsif Nkind (Renamed_Object (Ada_Entity))
                            = N_Explicit_Dereference then
                           null;
                        elsif Nkind (Renamed_Object (Ada_Entity))
                            = N_Indexed_Component then
                           null;
                        elsif Nkind (Renamed_Object (Ada_Entity))
                            = N_Unchecked_Type_Conversion then
                           null;
                        else
                           pragma Assert (False);
                           null;
         ...
         when N_Explicit_Dereference =>
            Evaluate_Expr (Prefix (Expr));
            --  if Top_Type = Type_Of (API_Class (
            --     Ada_Access_String)) then
            --     Pop_Type;
            --     Push_Type (JVM_Type (Standard_String));
            --  end if;
         ...
         when N_Unchecked_Type_Conversion =>
            --  If the target is a class or array type, then we
            --  grab the address of the first local and do a block
            --  copy, unless of course we are doing a conversion on
            --  the pointer itself.

            if JVM_Expr_Type (Expr) =
                  JVM_Expr_Type (Expression (Expr)) then
               Evaluate_Expr (Expression (Expr));
            ...
            then
               begin
                  ...
                  --  represented the same way and that we can get away
                  --  with not doing anything
                  if Ekind (Etype (Expr)) = E_Array_Subtype then
                     Evaluate_Expr (Expression (Expr));
                  elsif
                     Ekind (Etype (Expr)) = E_Access_Subtype or else
                     Ekind (Etype (Expr)) = E_Access_Type or else
                     Ekind (Etype (Expr)) = E_General_Access_Type or else
                     Ekind (Etype (Expr)) = E_Anonymous_Access_Type or else
                  ...
                  then
                     ...
                     Gen_Push_Int (
                        Esize (Etype (Expression (Expr))));
                     Gen_Block_Copy;
                     Push_Type (JVM_Expr_Type (Expr));
                  end if;
               end;
            --  mcc: can do conversions to these types in type-safe way!
            --  10/14/05
            elsif JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Byte_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Char_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Short_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Int_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Long_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Float_Kind or
                  JVM.Type_Kind (JVM_Expr_Type (Expr)) = JVM.Double_Kind
            then
               declare
                  Local2 : constant Local_Var_Id
                     := New_Local_Var
                     ("_conv_tmp2", JVM_Expr_Type (Expression (Expr)));
               begin
                  Evaluate_Expr (Expression (Expr));
            ...
            --  do a block copy
               ...
               begin
                  ...
                  Gen_Load_Local_Address (Local2);
--                  Gen_Load_Object_Field_Address (First_Field (
--                        JVM_Expr_Type (Expr)));
