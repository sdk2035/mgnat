...
package body JVM.Pool is
   ...
   type Pool_Item (Kind : CP_Tag := CONSTANT_Empty) is
      record
         ...
         Pool_Index : CP_Index;
         --  The physical index of the corresponding CP_Info record in
   ...
   --  This function returns an access value denoting the actual
   --  entry in Pool_Table that is associated with Id. Note that
   --  this access value is unsafe to use in conjunction with
   --  any operation that might involve adding entries to the
   --  table during the life of the access value. An exception
   --  is raised if Id is not in the current range of the table
   --  (but the range excludes the zeroth table index, which is
   --  reserved to denote the null value for Pool_Ids).
   ...
   function CP_Item_Id
     (Parent  : Class_Id;
      CP_List : Pool_Id;
      CP_Kind : CP_Tag := CONSTANT_Empty)
      return    Pool_Id;
   --  Returns a Pool_Id for a constant pool item belonging to class
   --  Parent and matching the CP_Tag indicated by CP_Kind (if CP_Kind
   --  /= CONSTANT_Empty) that exists in the list CP_List (there should
   --  be at most one); returns Null_Pool_Item if none exists.
   ...
   --  Creates a constant pool item with the given tag and returns
   --  a reference to it.
   ...
   begin
      New_Item.Next_Item := Null_Pool_Item;
   ...
   function Array_Item (Parent : Class_Id; A_Type : Type_Id) return Pool_Id is
      CP_Id : Pool_Id
        := CP_Item_Id (Parent, First_Pool_Ref (A_Type), CONSTANT_Class);
