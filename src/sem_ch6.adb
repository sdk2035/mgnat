...
package body Sem_Ch6 is
   ...
   begin
      if Result_Definition (N) /= Error then
         ...
         --  Subtype_Mark case
            ...
            --  mcc: 02/14/06 can we let incomplete valuetypes go?
   ...
   --  Start of processing for Analyze_Subprogram_Body
      Old_Current_Scope : constant Entity_Id := Current_Scope;  --  mcc
      Prev_Scope        : Entity_Id;                   --  mcc
   begin
      ...
      End_Scope;
      --  mcc:  well, there appears to be a bug here in that sometimes
      --  we don't get the scope we wanted after this end scope (like
      --  it created one somewhere that wasn't ended).  I got tired
      --  of trying to track this down, so I put a hack here to verify
      --  the scope level
      while Current_Scope /= Old_Current_Scope loop
         Prev_Scope := Current_Scope;
         End_Scope;
         if Current_Scope = Prev_Scope then
         ...
         if Nkind (Parameter_Type (Param_Spec)) /= N_Access_Definition then
            ...
            then
               ...
               if Is_Tagged_Type (Formal_Type) then
                  null;
               --  mcc: 02/14/06 can we let incomplete valuetypes go?
               elsif Ekind (Formal_Type) = E_Incomplete_Type
                  and then Namet.Get_Name_String (Chars (Formal_Type)) =
                     "valuetype"
               then
                  null;
