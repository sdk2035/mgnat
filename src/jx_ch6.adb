...
package body Jx_Ch6 is
   ...
   begin
      ...
      if Present (Comp_Spec) then
         Elab_Bool
           := New_Field (Subp_Class, Name ("_elabs_bool"),
             Boolean_Type, True);
         Elab_Method
      ...
      --  mcc: 02/09/06 Only do this if it acts as a spec (i.e. could
      --  really be the main program)
      ...
      then
         ...
         Binder_Class
           := New_Class (Name ("ada_" & Name_Buffer (1 .. Name_Len) & "_pkg"));
         Gen_Invoke_Static
   ...
   begin
         ...
         then
            ...
            if Chars (Subp) = Name_Op_Add then
               ...
               then
                  ...
                  --  The above comment is no longer true (mcc)

                  --  Gen_New_Object (API_Class (Lang_String));
                  --  Gen_Duplicate;
                  --  Slices are passed by copy to greatly simplify
                  --  handling their bounds. A temporary is created
                  --  and the sliced portion is
                  --  assigned. In the case of out and in out parameters,
                  --  we have to copy back after the call.

                  Test_For_Slice (First_Actual (Call), Is_Slice,
                     Slice_Prefix, Slice_Subt);

                  if Is_Slice then
                     Actual := First_Actual (Call);
                     declare
                        Slice_Temp : constant Local_Var_Id
                          := New_Local_Var ("_slicetmp1", JVM_Type (Actual));
                     begin
                        ...
                        Gen_Array_Subscript (Slice_Prefix,
                           Index_First (Slice_Subt));
               ...
               elsif Name_String (Chars (Etype (Subp))) =
               --   "string_access"
                  "string"
                 and then Ekind (Etype (First_Formal (Subp))) in Access_Kind
               then
                  declare
                     String_Tmp : constant Local_Var_Id
                       := New_Local_Var
                         ("_str_tmp", JVM_Type (Standard_String));
                     --     ("_str_tmp", JVM_Type (Etype (Subp)));
                  begin
                     Evaluate_Expr (First_Actual (Call));
                     Gen_Duplicate;
      ...
      if Is_Constructor (Subp) then
         ...
         if Present (Cntrl_Formal) then
            ...
            if Nkind (Actual) = N_Null then
               ...
               --  mcc:  Well, we used to call the default constructor
               --  then duplicate the object and call the real constructor
               --  later.  Not only is this inefficient, but it also
               --  breaks for objects that don't have a noarg constructor
               Constructor := True;
         ...
         elsif Name_String (Chars (Etype (Subp))) = "valuetype" then
            Value_Copy := New_Local_Var ("_valtemp",
                  JVM_Type (Etype (Subp)));
      ...
      --  mcc:  watch for valuetype return (not really dispatching)
      elsif Is_Dispatching_Operation (Subp) and then
            not (Convention (Subp) = Convention_Java and
      ...
      while Present (Actual) loop
         ...
         if Is_Slice then
            declare
               Slice_Temp : constant Local_Var_Id
                 := New_Local_Var ("_slicetmp2", JVM_Type (Actual));
         ...
         then
            --  Treepr.pn (Actual);  --  mcc: debugging
         ...
         --  that doesn't match Ada nondispatching semantics. ???
            ...
            --  the Ada call should be nondispatching, but give a warning.
               ...
               if Value_Copy /= Null_Local_Var then
                  Set_Class (Subp_Method, Class_Of_Type (Type_Of (
                     Value_Copy)));
                  Gen_Invoke_Method (Subp_Method);
                  Gen_Load_Local (Value_Copy);
               elsif (not (Superclass (Class_Of (Subp_Method)) = Null_Class))
                     and then Name_String (Name (Superclass (
                        Class_Of (Subp_Method))))
                     = "ValueType" then
                  Gen_Invoke_Method (Subp_Method);
      ...
      --  mcc: got rid of conversions for convention_java as this
      --  might be a constructor for an MSIL class
