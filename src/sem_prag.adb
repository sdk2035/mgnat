...
--  mcc:  05/17/06 also commented , for generics
--  mcc:  I commented these out so that I can put version and publickeytoken
--        info directly in the pragma Import's 3rd argument
--                             or else Get_Character (C) = ' '
      ...
      begin
         if No (Link_Arg) then
            if No (Ext_Arg) then
               if Ekind (Subprogram_Def) = E_Package and then
                  Nkind (Parent (Subprogram_Def)) =
                     N_Package_Specification and then
                  Present (Generic_Parent (Parent (Subprogram_Def))) then
                  Set_Interface_Name
                     (Subprogram_Def, Interface_Name (
         ...
         --  normal default.
            ...
            Start_String;
            --  mcc: we don't want this * in MSIL
      ...
      case Prag_Id is
         ...
         begin
            ...
            loop
               ...
               --  mcc: or else a valuetype
               if Ekind (Def_Id) = E_Function
                 and then
                 (Get_Name_String (Chars (Etype (Def_Id))) = "valuetype"
                  or else
   ...
   begin
      ...
      --  mcc: Also, for MGNAT, we check if there are package
      --  pragma Imports, which could generate the need for additional
      --  "assembly extern" statements.
      declare
         PA : constant List_Id := Pragmas_After (Aux_Decls_Node (N));
         P  : Node_Id;
         --  mcc: next two lines added
      ...
      begin
         if Present (PA) then
            ...
            while Present (P) loop
               --  mcc: this branch of if added for MGNAT
               if Chars (P) = Name_Import then
                  Arg1 := Empty;
                  Arg2 := Empty;
                  Arg3 := Empty;
                  Arg4 := Empty;
                  ...
                  if Present (Pragma_Argument_Associations (P)) then
                     Arg1 := First (Pragma_Argument_Associations (P));
                     ...
                     if Present (Arg1) then
                        ...
                        if Present (Arg2) then
                           ...
                           if Present (Arg3) then
                              ...
                              --  mcc: if the 3rd parameter begins with
                              --  '[', then it is the assembly name
                              --  otherwise, if the 3rd parameter begins
                              --  with .ver, then the 4th is the assembly
                              --  name
                              if Name_Len > 2 and then
                                    Name_Buffer (1) = '[' then
                                 JVM.MSIL_Emit.Add_Assembly (
                                    Name_Buffer (1 .. Name_Len));
                              elsif Name_Len > 4 and then
                                 Name_Buffer (1 .. 4) = ".ver" then
                                 Arg4 := Next (Arg3);
                                 if not Present (Arg4) then
                                    if Nkind (Arg3) =
                                       N_Pragma_Argument_Association then
                                       Error_Msg_N (
                                 ...
                                 end if;  -- not Present (Arg4)
                              end if;  -- Namelen > 2
                           end if;  -- Present (Arg3)
                        end if;
                     end if;
                  end if;
               elsif Chars (P) = Name_Suppress_All then
