...
package body Sem_Ch3 is
   ...
   begin
      ...
      begin
         ...
         then
            ...
            --  mcc: don't want N_Desig to remain unset??
            --  09/02/05
            else
               N_Desig := Desig;
            end if;
   ...
   begin
      ...
      if not Analyzed (T) then
         Set_Analyzed (T);
         --  mcc: I don't think you should be allowed to be frozen
         --  if we get here.  Make sure we're not.
         if Is_Frozen (T) then
            Set_Is_Frozen (T, False);
         end if;
   ...
   begin
      ...
      while Present (Elmt) loop
         ...
         if (Is_Abstract (Subp)
           ...
           --  mcc: 05/17/06
           --  the .NET libraries do bizarre things with interfaces
           and then Convention (T) /= Convention_Java
         ...
         then
            if Present (Alias (Subp)) then
               ...
               --  Ada 2005 (AI-391): In the case of an inherited function
               --  with a controlling result of the type, the rule does not
               --  apply if the type is a null extension (unless the parent
               --  function itself is abstract, in which case the function
               --  must still be overridden). The expander will generate an
               --  overriding wrapper function calling the parent subprogram
               --  (see Exp_Ch3.Make_Controlling_Wrapper_Functions).
               ...
               if Nkind (Type_Def) = N_Derived_Type_Definition
                 ...
                 and then
                   (Ada_Version < Ada_05
                      ...
                      or else Is_Abstract (Alias (Subp))
   ...
   begin
      ...
      then
         ...
         return Chars (Defining_Identifier (First_Comp)) = Name_uParent
           and then not Present (Next (First_Comp));
