-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
pragma Extensions_Allowed(On);
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
with MSSyst.Net.Mail.SmtpFailedRecipientException;
with MSSyst.Runtime.Serialization.ISerializable;
with MSSyst.Runtime.InteropServices.uException;
with type MSSyst.Object.Ref is access;
with type MSSyst.Collections.IDictionary.Ref is access;
with type MSSyst.String.Ref is access;
with type MSSyst.Exception_k.Ref is access;
with type MSSyst.Net.Mail.SmtpFailedRecipientException.Ref_array is access;
with MSSyst.Net.Mail.SmtpStatusCode;
with type MSSyst.Reflection.MethodBase.Ref is access;
with type MSSyst.Runtime.Serialization.SerializationInfo.Ref is access;
with type MSSyst.Runtime.Serialization.StreamingContext.Valuetype is tagged;
with type MSSyst.Type_k.Ref is access;
package MSSyst.Net.Mail.SmtpFailedRecipientsException is
   type Typ;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   type Typ is new MSSyst.Net.Mail.SmtpFailedRecipientException.Typ
         and MSSyst.Runtime.Serialization.ISerializable.Typ
         and MSSyst.Runtime.InteropServices.uException.Typ
   with record
      null;
   end record;
   function new_SmtpFailedRecipientsException(
      This : Ref := null) return Ref;
   function new_SmtpFailedRecipientsException(
      This : Ref := null;
      message : MSSyst.String.Ref) return Ref;
   function new_SmtpFailedRecipientsException(
      This : Ref := null;
      message : MSSyst.String.Ref;
      innerException : MSSyst.Exception_k.Ref) return Ref;
   function new_SmtpFailedRecipientsException(
      This : Ref := null;
      message : MSSyst.String.Ref;
      innerExceptions : MSSyst.Net.Mail.SmtpFailedRecipientException.Ref_array) return Ref;
   function get_InnerExceptions(
      This : access Typ) return MSSyst.Net.Mail.SmtpFailedRecipientException.Ref_array;
   procedure GetObjectData(
      This : access Typ;
      serializationInfo : MSSyst.Runtime.Serialization.SerializationInfo.Ref;
      streamingContext : MSSyst.Runtime.Serialization.StreamingContext.Valuetype);
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_SmtpFailedRecipientsException);
   pragma Import(MSIL,get_InnerExceptions,"get_InnerExceptions");
   pragma Import(MSIL,GetObjectData,"GetObjectData");
end MSSyst.Net.Mail.SmtpFailedRecipientsException;
pragma Extensions_Allowed(Off);
pragma Import(MSIL,MSSyst.Net.Mail.SmtpFailedRecipientsException,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System]System.Net.Mail.SmtpFailedRecipientsException");
