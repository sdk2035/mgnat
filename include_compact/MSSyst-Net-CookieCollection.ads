-- Autogenerated by MSIL2Ada v. 2
-- By: Martin C. Carlisle
--     Department of Computer Science
--     US Air Force Academy
--     carlislem@acm.org
pragma Extensions_Allowed(On);
with MSSyst.Object;
with MSIL_Types;
use MSIL_Types;
with MSSyst.Object;
with MSSyst.Collections.ICollection;
with type MSSyst.Net.Cookie.Ref is access;
with type MSSyst.Array_k.Ref is access;
with type MSSyst.Net.Cookie.Ref_array is access;
with type MSSyst.String.Ref is access;
with type MSSyst.Collections.IEnumerator.Ref is access;
with type MSSyst.Type_k.Ref is access;
package MSSyst.Net.CookieCollection is
   type Typ;
   type Ref is access all Typ'Class;
   type Ref_addrof is access all Ref;
   type Ref_Arr is array(Natural range <>) of Ref;
   type Ref_Array is access all Ref_Arr;
   type Ref_Array_addrof is access all Ref_Array;
   type Typ is new MSSyst.Object.Typ
         and MSSyst.Collections.ICollection.Typ
   with record
      null;
   end record;
   function new_CookieCollection(
      This : Ref := null) return Ref;
   procedure Add(
      This : access Typ;
      cookie : MSSyst.Net.Cookie.Ref);
   procedure Add(
      This : access Typ;
      cookies : MSSyst.Net.CookieCollection.Ref);
   procedure CopyTo(
      This : access Typ;
      array_k : MSSyst.Array_k.Ref;
      index : Integer);
   procedure CopyTo(
      This : access Typ;
      array_k : MSSyst.Net.Cookie.Ref_array;
      index : Integer);
   function get_Count(
      This : access Typ) return Integer;
   function get_IsReadOnly(
      This : access Typ) return Standard.Boolean;
   function get_IsSynchronized(
      This : access Typ) return Standard.Boolean;
   function get_Item(
      This : access Typ;
      name : MSSyst.String.Ref) return MSSyst.Net.Cookie.Ref;
   function get_Item(
      This : access Typ;
      index : Integer) return MSSyst.Net.Cookie.Ref;
   function get_SyncRoot(
      This : access Typ) return MSSyst.Object.Ref;
   function GetEnumerator(
      This : access Typ) return MSSyst.Collections.IEnumerator.Ref;
private
   pragma Convention(MSIL,Typ);
   pragma MSIL_Constructor(new_CookieCollection);
   pragma Import(MSIL,Add,"Add");
   pragma Import(MSIL,CopyTo,"CopyTo");
   pragma Import(MSIL,get_Count,"get_Count");
   pragma Import(MSIL,get_IsReadOnly,"get_IsReadOnly");
   pragma Import(MSIL,get_IsSynchronized,"get_IsSynchronized");
   pragma Import(MSIL,get_Item,"get_Item");
   pragma Import(MSIL,get_SyncRoot,"get_SyncRoot");
   pragma Import(MSIL,GetEnumerator,"GetEnumerator");
end MSSyst.Net.CookieCollection;
pragma Extensions_Allowed(Off);
pragma Import(MSIL,MSSyst.Net.CookieCollection,
   ".ver 2:0:0:0 .publickeytoken=( b7 7a 5c 56 19 34 e0 89 )",
   "[System]System.Net.CookieCollection");
