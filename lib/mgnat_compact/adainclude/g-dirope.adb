...
package body GNAT.Directory_Operations is
   ...
   function Getenv (Name : String) return String renames
   ...
   procedure Close (Dir : in out Dir_Type) is
      ...
      function closedir (Dir : Dir_Type) return Integer;
   ...
   is
      Environment_Variable_Char : constant Character := '%';
      ...
      begin
         ...
         if P = '%' or else Path (K) = '{' then
            ...
            if Characters.Handling.Is_Letter (Path (E)) then
               ...
               declare
                  Env : OS_Lib.String_Access := new String'(
                     Getenv (Path (K .. E)));
   ...
   procedure Get_Current_Dir (Dir : out Dir_Name_Str; Last : out Natural) is
      Path_Len : aliased Natural := Max_Path;
      Buffer   : aliased String (Dir'First .. Dir'First + Max_Path + 1);

      function Local_Get_Current_Dir
        (Dir    : System.Address) return Integer;
      pragma Import (C, Local_Get_Current_Dir, "__gnat_get_current_dir");

   begin
      Path_Len := Local_Get_Current_Dir (Buffer'Address);
   ...
   function Is_Open (Dir : Dir_Type) return Boolean is
      function Is_Open (Dir : Dir_Type) return Integer;
      pragma Import (C, Is_Open, "__gnat_dir_isopen");
   begin
      return Dir /= Null_Dir
        and then Is_Open (Dir) = 1;
   end Is_Open;
   ...
   is
      ...
      --  we don't want it to generate native int, but Object
      type String_Address is new System.Address;

      function readdir_gnat
        (Directory : Dir_Type;
         Buffer    : String_Address) return Integer;
   ...
   begin
      ...
      Filename_Len :=
        readdir_gnat (Dir, String_Address (Buffer'Address));
