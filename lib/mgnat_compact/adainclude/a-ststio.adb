...
--  This is the JGNAT-specific version of the body of Ada.Streams.Stream_IO.
--  The only difference between this and the GNAT version is the calls to
--  the method GNAT_libc.set_file_class in the Create and Open procedures.
...
package body Ada.Streams.Stream_IO is
   ...
   is
      ...
      procedure Set_File_Class (Class : Character);

      pragma Import (C, Set_File_Class, "set_file_class");

   begin
      --  This call is currently needed by the JGNAT implementation
      --  in order to communicate to GNAT_libc that a stream file
      --  being created, which will be represented using a Java
      --  RandomAccessFile.
   ...
   begin
      --  This call is currently needed by the JGNAT implementation
      --  in order to communicate to GNAT_libc that a stream file
      --  being opened, which will be represented using a Java
      --  RandomAccessFile.
